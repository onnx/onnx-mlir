...
def print_usage(error_msg="", options=False, usage=False, file_format=False):
    ...
    if usage: 
        ...
        dprint(" * Check a failing function X (\"-f X\"): \"fixLitTest [-td] -f X test.mlir\".")
...
def main(argv):
    ...
    for line in open(lit_test_filename, 'r'):
        ...
        if m is not None:
            ...
            if fct_between_delimiters > 0:
                print_usage('Got too many function bodies between "// -----" command starting with ' 
                            + curr_segment_fct_name, file_format=True)
    ...
    if len(segment_text) < 2:
        print_usage('Expected at least 2 segments (text between "// -----"): one for RUN command and one for a function.', options=False)
