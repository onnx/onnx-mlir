...
def print_usage(msg = ""):
    if msg:
        print("Error:", msg, "\n")
    print("make-report -[svh] [-c <compile_log>] [-r <run_log>] [-l <num>] [-p <op regexp>]")
    print("")
    print("Usage: Report statistics on compiler and runtime characteristics of onnx ops.")
    print("")
    print("Compile time statistics are collected from the `onnx-mlir` compiler output")
    print("with the option `--onnx-op-report` equal to `Simd` or other supported sub-options.")
    print("")
    print("Runtime statistics are collected from the runtime output of a model compiled.")
    print("with the option `--profile-ir` equal to `Onnx` or other supported sub-options")
    print("")
    print("When both compile time and runtime statistics are provided, the script")
    print("will correlate the performance metrics with data gathered at compile time")
    print("")
