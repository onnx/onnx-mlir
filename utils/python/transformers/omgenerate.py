...
class OMGeneration:
    ...
    def generate(
        ...
        **kwargs
        ...
        if generation_config is not None:
            ...
            max_new_tokens = max_new_tokens or getattr(generation_config, 'max_new_tokens', None)
            ...
            eos_token_id = eos_token_id or getattr(generation_config, 'eos_token_id', None)
        ...
        attention_mask = attention_mask if attention_mask is not None else np.ones_like(input_ids, dtype=np.int64)
        ...
        while cur_len < max_length:
            ...
            print(f"DEBUG - next_tokens dtype: {next_tokens.dtype}")
            print(f"DEBUG - next_tokens: {next_tokens}")
    ...
    def _sample_next_tokens(
        ...
        next_tokens = np.array([rng.choice(vocab_size, p=probs[i]) for i in range(batch_size)])
