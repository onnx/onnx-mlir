...
def get_args():
    ...
    parser.add_argument(
        "-i", "--input", help="Provide the NAME input log file to parse, e.g. a file " 
        "generated by an 'onnx-mlir --mlir-print-after-all' command.",
        metavar="NAME"
    )
    parser.add_argument(
        "-p", "--pass-name", help="Name of the pass to list. When regexp matches "
        "multiple passes, use -n option to indicate which match to select.",
        metavar="REGEX"
)
    parser.add_argument(
        "-n", "--num", help="Id NUM of the pass to list when -p is not specified. " 
        "Otherwise, list the NUM(th) match returned by the -p regex string.",
        metavar="NUM"
    )
    parser.add_argument(
        "-a", "--after", help="Print the pass NUM(th) after the pass indicated by the  "
...
def locate_pass(name, num):
    ...
    num_ids = len(ids)
    if num_ids == 0:
        usage(f"pass {name} not found; please check the name of the pass you are looking for.")
    if num_ids>1:
        if num:
            n = int(num)
            if n<0 or n>= num_ids:
                usage(f"Provided a -n {n} number that is not in the range [0..{num_ids}).")
            return ids[n]
        print(f"Pass {name} is ambiguous and matched with the following ids:")
        print("  ", ids)
        print("  Please choose which one with the -n <id> option to select a unique pass.")
        exit(1)
    return ids[0]
