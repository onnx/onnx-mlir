...
- Using `onnx-mlir --nnpa-save-config-file=cfg.json model.onnx`.
...
2. Using `--nnpa-save-config-file=cfg.json`
...
- by passing a JSON file for device placement to the compiler by using `--nnpa-load-config-file=json`.
...
For the later option, users can obtain a template file from `--nnpa-save-config-file`, and use it as the starting point of modification.
