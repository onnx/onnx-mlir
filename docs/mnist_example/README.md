...
onnx-mlir provides a multi-thread safe parallel compilation mode. Whether each thread is given a name or not by the user, onnx-mlir is multi-threaded safe. If you need to give a name to a thread, use the `-menvVarName` keyword and an example can be found as follows.

```bash
onnx-mlir -O3 -menvVarName mnistwith03 [--EmitLib] mnist.onnx -o mnist03
```
