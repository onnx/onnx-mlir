...
1. The shapred library to compile onnx-mlir models is generated by `PyOMCompileSession` (src/Compiler/PyOMCompileSession.hpp) and build as a shared library to `build/Debug/lib/PyOMCompile.cpython-<target>.so`.
...
4. The shared library to compile onnx-mlir models is generated by `PyOMCompileSession` (src/Runtime/PyOMCompileSession.hpp) and built as a shared library to `build/Debug/lib/PyOMCompileC.cpython-<target>.so`.
5. The Python library to compile onnx-mlir models (src/Runtime/python/PyCompileAndRuntime.py). This library takes an .onnx file and the options as inputs, it will load it and then compile and run it.
...
ln -s <path to the shared library to copmpile onnx-mlir models>(e.g. `build/Debug/lib/PyOMCompile.cpython-<target>.so`) .
