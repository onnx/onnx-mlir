...
def __init__(self, shared_lib_path: str, use_default_entry_point: bool):
    """
    Constructor for an execution session.
...
def run(self, input: List[ndarray]) -> List[ndarray]:
    """
    Method to run the current inference model given specific inputs.
...
def input_signature(self) -> str:
    """
    Method to get the input signature of the current inference model.
...
def output_signature(self) -> str:
    """
    Method to get the output signature of the current inference model.
...
def entry_points(self) -> List[str]:
    """
    Method to list the available model entry points in the current executable.
    Returns:
...
def set_entry_point(self, name: str):
    """
    Method to set the current model to the given entry point.
...
Typically, a compiler object is created for a given model by giving it the file name of the ONNX model. Then, compiler options can be set/reset to steer the compilation to
generate the desired executable. Finally, the compilation itself is performed by calling
the `compile()` command where the user passes the output file name (without extension) as well as the desired compilation target. Typical targets are `emit_lib` or `emit_jni`.
