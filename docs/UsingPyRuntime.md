...
These utilities are implemented by the `CompilerSession` compiler interface
(src/Runtime/CompilerSession.h) and the `ExecutionSession` class
...
3. The shared library to compile onnx-mlir models is generated by `PyOMCompileSession` (src/Runtime/PyOMCompileSession.hpp) and built as a shared library to `build/Debug/lib/PyOMCompileC.cpython-<target>.so`.
...
# Load onnx model, compile, and create OMCompileSession object.
try:
    compiler = OMCompileSession('./mnist.onnx', '-O3 -o mnist')
...
The `PyOMCompile` module exports the `OMCompileSession` class to drive the
