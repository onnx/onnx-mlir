...
jenkins_rest_api_url = {
    "s390x": "http://localhost:8080/jenkins",
...
LLVM_PROJECT_DOCKERFILE = os.path.join(
    workspace_dir, "docker", "Dockerfile.llvm-project"
)
...
ONNX_MLIR_DOCKERFILE = os.path.join(workspace_dir, "docker", "Dockerfile.onnx-mlir-dev")
...
INIT_WATCH_STATE = {
    ...
    "build_history": [
        {
            "head": {"sha1": "", "date": EPOCH0, "stat": {}, "mesg": ""},
...
def get_remote_repo_sha1_history(
    ...
    try:
        ...
        index_newest = (
            sha1_history.index(commit_sha1_newest) if commit_sha1_newest else 0
        )
        ...
        for i, (sha1, date, stat, mesg) in enumerate(
            zip(
                sha1_history[index_newest:index_oldest],
...
def get_local_repo_sha1_date(local_repo):
    ...
    repo_sha1_date = (
        datetime.utcfromtimestamp(repo.head.commit.committed_date).isoformat() + "Z"
    )
...
def remove_recent_image(curr_state, watch_state, key, reset=False):
    ...
    if build:
        remove_docker_images(
            [
                LLVM_PROJECT_WATCH_IMAGE + ":" + build,
...
def cleanup_old_builds(curr_recent):
    ...
    job_info = jenkins_server.get_job_info(LLVM_WATCH_JOB_NAME, fetch_all_builds=True)
...
def check_running_job():
    ...
    for build in running_builds:
        ...
        if build["name"] == ONNX_MLIR_JOB_NAME or (
            build["name"] == LLVM_WATCH_JOB_NAME and build["number"] < int(build_number)
        ):
            logging.info(
                "Active build(s) find in {}, skip this run".format(build["name"])
            )
            return True
...
def compute_range_build_next():
    ...
    #
    llvm_onnx_mlir = extract_pattern_from_file(
        LLVM_PROJECT_SHA1_FILE, LLVM_PROJECT_SHA1_REGEX
    )
    logging.info("ONNX-MLIR currently using LLVM commit {}".format(llvm_onnx_mlir))
    ...
    retrieve_until_sha1 = (
        watch_state["llvm_history_github"]["history"][0]["sha1"]
        if watch_state["llvm_history_github"]["history"]
        else llvm_onnx_mlir
    )

    new_llvm_history = get_remote_repo_sha1_history(
        LLVM_PROJECT_GITHUB_URL, github_repo_access_token, retrieve_until_sha1
    )
    ...
    llvm_onnx_mlir_index = next(
        (
            index
            for (index, hist) in enumerate(new_llvm_history["history"])
            if hist["sha1"] == llvm_onnx_mlir
        ),
        new_llvm_history_len,
    )
    logging.info(
        "commit {} index={}, history length={}".format(
    ...
    if curr_status:
        ...
        if llvm_tail["date"] > curr_head["date"]:
            ...
            logging.info(
                (
                    "curr: {}, llvm_tail {} > curr_head/failed {}\n" + "next range:\n{}"
                ).format(
                    ...
                    json.dumps(
                        [
                            {
                                "index": (
                                    llvm_history["index"][next_head["sha1"]]
                                    if next_head
                                    else -1
                                ),
                                "head": next_head,
                            ...
                            {
                                "index": (
                                    llvm_history["index"][next_tail["sha1"]]
                                    if next_tail
                                    else -1
                                ),
                                "tail": next_tail,
        ...
        #
            ...
            if curr_failed["sha1"]:
                ...
                logging.info(
                    (
                        "curr: {}, llvm_tail {} > curr_middle {}, w/ failure\n"
                        + "next range:\n{}"
                    ).format(
                        "success" if curr_status else "failure",
                        json.dumps(llvm_tail),
            ...
            # no failure yet: [llvm_head, llvm_tail]
                ...
                logging.info(
                    (
                        "curr: {}, llvm_tail {} > curr_middle {}, no failure\n"
                        + "next range:\n{}"
                    ).format(
                        "success" if curr_status else "failure",
                        json.dumps(llvm_tail),
        ...
        if next_size:
            ...
            logging.info(
                "converged: {}\nnext middle:\n{}".format(
                    ...
                    json.dumps(
                        {
                            "index": next_history["index"][next_middle["sha1"]],
                            "middle": next_middle,
                            "size": next_size,
                        },
                        indent=JSON_DUMPS_INDENT,
    ...
    #
        ...
        if llvm_tail["date"] > curr_middle["date"]:
            ...
            logging.info(
                (
                    "curr: {}, llvm_tail {} > curr_middle/failed {}\n"
                    + "next range:\n{}"
                ).format(
                    "success" if curr_status else "failure",
                    json.dumps(llvm_tail),
