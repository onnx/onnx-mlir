...
MEMORY_IN_GB = (os.sysconf('SC_PAGE_SIZE') * os.sysconf('SC_PHYS_PAGES') /
                (1024.**3))
...
LLVM_PROJECT_IMAGE = {
    'dev': docker_static_image_name,
    'usr': docker_shared_image_name
}
PROJECT_IMAGE = {'dev': docker_dev_image_name, 'usr': docker_usr_image_name}
PROJECT_DOCKERFILE = {
    'dev': 'docker/Dockerfile.' + github_repo_name + '-dev',
    'usr': 'docker/Dockerfile.' + github_repo_name
}
PROJECT_LABELS = [
    github_repo_name2 + '_sha1', github_repo_name2 + '_sha1_date',
...
def get_proj_repo_info(image_type, local_repo):
    ...
    exp_proj_repo_filter = {
        'label': [
            github_repo_name2 + '_sha1=' + exp_proj_repo_sha1,
            github_repo_name2 + '_dockerfile_sha1=' +
            exp_proj_repo_dockerfile_sha1,
...
def get_access_token(user_name, image_name, action, login_name, login_token):
    resp = requests.get(
        url=('https://auth.docker.io/token' + '?service=registry.docker.io' +
             '&scope=repository:' + (user_name + '/' if user_name else '') +
             image_name + ':' + action),
        auth=(login_name, login_token))
    resp.raise_for_status()
...
# Make REST call to get the v1 or v2 manifest of an image from
# public docker registry
def get_image_manifest_public(user_name,
                              image_name,
                              image_tag,
                              schema_version,
                              login_name,
    ...
    if not access_token:
        access_token = get_access_token(user_name, image_name, 'pull',
                                        login_name, login_token)
    # Get manifest
    resp = requests.get(url=('https://registry-1.docker.io/v2/' +
...
def get_remote_image_labels(host_name, user_name, image_name, image_tag,
    ...
    try:
        ...
        resp = (
            # private docker registry
            get_image_manifest_private(host_name, user_name, image_name,
                                       image_tag, 'v1', login_name,
                                       login_token) if host_name else
            # public docker registry
            get_image_manifest_public(user_name, image_name, image_tag, 'v1',
                                      login_name, login_token))
        ...
        logging.info('remote image %s labels: %s', image_full,
...
def remove_dependent_containers(image):
    containers = docker_api.containers(filters={'ancestor': image},
                                       all=True,
                                       quiet=True)
    for container in containers:
        try:
            ...
            logging.info('  Labels %s',
                         str(container_info['Config']['Labels']))
...
def build_private_project(image_type, exp):
    ...
    if not id:
        ...
        try:
            labels = get_remote_image_labels(host_name, user_name, image_name,
                                             cpu_arch, image_labels,
                                             login_name, login_token)

            # Image in registry has expected onnx-mlir commit sha1, pull and
            # tag it with pull request number for our private use.
            if (labels and labels[github_repo_name2 + '_sha1']
                    == exp[github_repo_name2 + '_sha1']):

                for line in docker_api.pull(image_repo,
                                            tag=cpu_arch,
                                            stream=True,
                                            decode=True):
                    print((line['id'] + ': ' if 'id' in line
                           and 'progress' not in line else '') +
                          (line['status'] + '\n' if 'status' in line
                           and 'progress' not in line else ''),
                          end='',
                          flush=True)
        ...
        for line in docker_api.build(
                ...
                buildargs={
                    'BASE_IMAGE':
                    base_image_repo + ':' + base_image_tag,
                    'NPROC':
                    NPROC,
                    GITHUB_REPO_NAME2 + '_SHA1':
                    exp[github_repo_name2 + '_sha1'],
                    GITHUB_REPO_NAME2 + '_SHA1_DATE':
                    exp[github_repo_name2 + '_sha1_date'],
                    GITHUB_REPO_NAME2 + '_DOCKERFILE_SHA1':
                    exp[github_repo_name2 + '_dockerfile_sha1'],
                    GITHUB_REPO_NAME2 + '_PR_NUMBER':
                    github_pr_number,
                    GITHUB_REPO_NAME2 + '_PR_NUMBER2':
                    github_pr_number2
                }):
            ...
            if 'error' in line:
                ...
                if layer_sha256:
                    ...
                    logging.info('tagging %s -> %s for debugging', image_layer,
                                 image_full)
                    docker_api.tag(image_layer,
                                   image_repo,
                                   image_tag,
                                   force=True)
                else:
