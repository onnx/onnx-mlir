...
# Class to compile ONNX or MLIR models to shared libraries (.so files)
# using either a local compiler or a container image.
class CompileSession:
    ...
    def handleParameters(self, model_path, **kwargs):
        ...
        # By default, the compiled model is in the directory of the model
        # Perhaps, the current directory?
    ...
    def get_compiled_model_path(self):
        """Return the path to the compiled .so file"""
        return self.compiled_model


def CompileWithContainer(model_path, **kwargs):
