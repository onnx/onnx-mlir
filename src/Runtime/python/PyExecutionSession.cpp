...
std::vector<py::array> PyExecutionSession::pyRunImplementation(
  ...
  for (size_t argId = 0; argId < inputsPyArray.size(); argId++) {
    ...
    if (dtype == ONNX_TYPE_STRING) {
      ...
      // omTensorPrint("PyExecutionSession::pyRun: %t %d", inputOMTensor);
...
std::string PyExecutionSession::pyCompiledModelPath() const {
...
void PyExecutionSession::pyPrintInstrumentation() {
