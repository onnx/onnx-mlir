...
const std::string *ExecutionSession::queryEntryPoints(
  ...
  if (!isInitialized)
    throw std::runtime_error(reportInitError());
  return reinterpret_cast<const std::string *>(_queryEntryPointsFunc(numOfEntryPoints));
}
...
std::vector<OMTensorUniquePtr> ExecutionSession::run(
  ...
  auto *wrappedInput = omTensorListCreate(omts.data(), static_cast<int64_t>(omts.size()));
