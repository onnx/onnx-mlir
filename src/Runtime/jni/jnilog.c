...
static char *log_level_name[] = { "trace", "debug", "info", "warning", "error", "fatal" };
...
/* Initialize log system. Set default log level and file or use environment variables
 * ONNX_MLIR_JNI_LOG_LEVEL and ONNX_MLIR_JNI_LOG_FILE, respectively. */
static void log_init() {
  ...
  int level;
  if (strlevel && (level = get_log_level_by_name(strlevel)) != -1) log_level = level;

  log_fp = stderr;
  char *strfname = getenv("ONNX_MLIR_JNI_LOG_FILE");
  FILE *fp;
  if (strfname && (fp = get_log_file_by_name(strfname))) log_fp = fp;
...
/* Generic log routine */
void log_printf(int level, char *file, const char *func, int line, char *fmt, ...) {
  if (!log_initd) log_init();
  ...
  fprintf(log_fp, "[%s][%s]%s:%s:%d ",
	  buf, log_level_name[level], basename(file), func, line);
