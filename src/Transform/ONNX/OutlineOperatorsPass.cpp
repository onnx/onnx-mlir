...
public:
  ...
  class FlattenPattern : public RewritePattern {
  public:
    FlattenPattern(
        StringRef opName, PatternBenefit benefit, MLIRContext *context)
        : RewritePattern(opName, benefit, context) {}
    //  FlattenPattern(PatternBenefit benefit, MLIRContext *context)
    ...
    LogicalResult matchAndRewrite(
      ...
      std::cout << "**** RewritePattern - inside match and rewrite: "
                << "Operation is " << opName << std::endl;
      flattenSubgraph(rewriter, op);
      return success();
    }
  };
  ...
  void applyOutlinePatternDriver(Operation *op, RewritePatternSet &patterns) {
    ...
    op->walk([&](Operation *op) {
      LogicalResult result = applicator.matchAndRewrite(
          op, rewriter,
          [op](const Pattern &pat) -> bool {
          ...
          [](const Pattern &pat) { std::cout << "failing ..." << std::endl; },
  ...
  static void flattenSubgraph(PatternRewriter &rewriter, Operation *op) {
    ...
    ++outlineCount;
    std::cout << "   entered flattenSubgraph: funcName=" << funcName
              << std::endl;
    ...
    std::cout << "      number of results " << results.size()
              << "  number of inputs " << inputVals.size() << std::endl;
    TypeRange sgTRange(results);
    auto regions = op->getRegions();
    // auto r = regions.iterator();
    // Region &sgRegion = std::next(regions.iterator());;
    Region &sgRegion = *regions.begin();
    auto &blocks = sgRegion.getBlocks();
    Block &sgBlock = *blocks.begin();
    // Block &sgBlock = sgRegion.getBlocks()[0];
  ...
  static void genOutlinedFunction(PatternRewriter &rewriter, Operation *op) {
    ...
    // const llvm::StringRef name=funcName;
    ++outlineCount;
    std::cout << "   entered genOutlinedFunction: funcName=" << funcName
              << std::endl;
