...
char *getArrayFromAttributeOrBuffer(
  ...
  if (bufferIDAttr) {
    ...
    // res = allocateBufferFor(constOp.getResult().getType(),
    // /*useMaxSize=*/true);
...
// template <typename T>
// void IterateConstPropTranspose(char *constArray, ArrayRef<int64_t>
// constShape,
...
//    // Use maximum size (double or int64_t) to avoid the precision loss.
//    char *resArray = allocateBufferFor(replacingTypes[i],
//    /*useMaxSize=*/true); resBuffers.emplace_back(resArray);
//  }
//
//  // Do splitting
