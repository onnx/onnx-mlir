...
llvm::Optional<std::string> getEnvVar(const std::string &name) {
...
int Command::exec(const std::string &wdir) const {
...
static void loadMLIR(const std::string &inputFilename,
...
static int genJniObject(const mlir::OwningOpRef<ModuleOp> &module,
    const std::string &jniSharedLibPath, const std::string &jniObjPath) {
...
// Return 0 on success, error code on failure.
static int genSharedLib(const std::string &sharedLibNameWithExt,
...
static int genJniJar(const mlir::OwningOpRef<ModuleOp> &module,
    const std::string &modelSharedLibPath, const std::string &modelJniJarPath) {
...
static int compileModuleToSharedLibrary(
    const mlir::OwningOpRef<ModuleOp> &module,
    const std::string &outputNameNoExt, std::string &libNameWithExt) {
...
// Return 0 on success, error code on failure
static int compileModuleToJniJar(const mlir::OwningOpRef<ModuleOp> &module,
    const std::string &outputNameNoExt) {
  std::string modelObjNameWithExt;
  int rc = compileModuleToObject(module, outputNameNoExt, modelObjNameWithExt);
  if (rc != CompilerSuccess)
...
// Return 0 on success, error code on error.
int outputCode(mlir::OwningOpRef<ModuleOp> &module,
    const std::string &filenameWithExt, int64_t largeElementLimit) {
  std::string errorMessage;
...
// Return 0 on success, error code on failure.
static int emitOutputFiles(const std::string &outputNameNoExt,
...
static int setupModule(mlir::OwningOpRef<ModuleOp> &module,
    mlir::MLIRContext &context, const std::string &outputNameNoExt) {
