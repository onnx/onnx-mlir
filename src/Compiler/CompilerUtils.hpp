...
//
// // Return 0 on success. These functions are not thread-safe and should be
// called
// // by a single program thread.
// int setCompilerOption(const onnx_mlir::OptionKind kind, const std::string
// &val); int setCompilerOptions(const CompilerOptionList &list);
//
