...
/// A named pass entry: human-readable name + factory to create a fresh instance.
struct XmcPassEntry {
  std::string name;
...
static std::vector<XmcPassEntry> getXmcPassEntries() {
  ...
  add("convert-scast-pair-to-requantize",               onnx_mlir::createConvertSCastPairToRequantizePass);
  // clang-format on
...
static void dumpMLIR(mlir::ModuleOp module, const std::string &passName,
  ...
  if (mkdirEc) {
    llvm::errs() << "  [XMC-DEBUG] Warning: could not create dir '"
...
public:
  ...
  XmcDebugInstrumentation(const std::string &dir,
      const std::vector<XmcPassEntry> &passes) {
  ...
  void runAfterPass(Pass *pass, Operation *op) override {
    ...
    llvm::errs() << "  [XMC-DEBUG] Pass [" << passIndex << "] '"
                 << displayName << "' completed in " << durationMs << " ms"
...
void addXmcMlirPasses(mlir::PassManager &pm, OnnxToMlirOptions opts) {
  ...
  if (opts.dumpMlirAfterEachXmcPass) {
    pm.addInstrumentation(std::make_unique<XmcDebugInstrumentation>(
        opts.xmcOutputDir, passes));
  }
