...
#include "src/Transform/ZHigh/Stickify/stickify.h"
...
/// Macros from AI-on-Z/src/zdnn/zdnn_private.h
...
// End - Macros from AI-on-Z/src/zdnn/zdnn_private.h

// Functions from AI-on-Z/zdnn/get.c
...
#undef CASE_RTN_SIZE
} // End - Functions from AI-on-Z/zdnn/get.c

// Functions from AI-on-Z/src/zdnn/malloc4k.c
...
// End - Functions from AI-on-Z/src/zdnn/malloc4k.c

// Functions from AI-on-Z/zdnn/utils.c
uint64_t get_num_elements(const zdnn_ztensor *ztensor, elements_mode mode) {
  ...
  return num_elements;
} // End - Functions from AI-on-Z/zdnn/utils.c

// Functions from AI-on-Z/src/zdnn/allochelper.c
...
} // End - Functions from AI-on-Z/src/zdnn/allochelper.c

// Functions from AI-on-Z/src/zdnn/tensor_desc.c
...
zdnn_status verify_transformed_descriptor(const zdnn_tensor_desc *tfrmd_desc) {
  ...
  for (int i = 0; i < ZDNN_MAX_DIMS; i++) {
    if (!dims_ptr[i] || dims_ptr[i] > MAXIMUM_DIMENSION_INDEX_SIZE) {
      return ZDNN_INVALID_SHAPE;
    }
  }

  // is stick area size above the limit?
  if (getsize_ztensor(tfrmd_desc) > MAXIMUM_TENSOR_SIZE) {
    return ZDNN_INVALID_SHAPE;
  }
...
zdnn_status generate_transformed_desc_concatenated(
  ...
  return ZDNN_STATUS_OK;
} // End - Functions from AI-on-Z/src/zdnn/tensor_desc.c

// Functions from AI-on-Z/src/zdnn/init_ztensor.c
...
} // End - Functions from AI-on-Z/src/zdnn/init_ztensor.c

// Functions from AI-on-Z/src/zdnn/stickify.c
...
uint32_t convert_data_format(void *input_data, zdnn_data_types in_data_fmt,
  ...
  if (out_data_fmt == ZDNN_DLFLOAT16) {
    switch (in_data_fmt) {
    case FP16:
      num_fields_converted = fp16_to_dlf16(
    ...
    switch (out_data_fmt) {
    case FP16:
      num_fields_converted = dlf16_to_fp16(
          (uint16_t *)input_data, (uint16_t *)output_data, num_fields);
      break;
    case FP32:
...
///          ZDNN_CONVERT_FAILURE
///
/// \see AI-on-Z/src/zdnn/stickify.c
zdnn_status stickify(zdnn_ztensor *ztensor, ...) {
  ...
  va_end(argptr);
  return status;
} // End - Functions from AI-on-Z/src/zdnn/stickify.c
