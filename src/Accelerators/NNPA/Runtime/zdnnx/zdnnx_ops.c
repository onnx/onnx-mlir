...
static inline zdnn_status call_unary_op(const zdnn_ztensor *input,
...
static inline zdnn_status call_binary_op(const zdnn_ztensor *input_a,
...
zdnn_status zdnn_matmul_op_ext(const zdnn_ztensor *input_a,
  ...
  if (get_num_procs() == 1)
    status = seq_matmul(
        input_a, input_b, input_c, op_type, output, /*is_bcast=*/false);
  else
    status = omp_matmul(input_a, input_b, input_c, op_type, output,
        /*is_bcast=*/false);
#else
  status = seq_matmul(
      input_a, input_b, input_c, op_type, output, /*is_bcast=*/false);
...
zdnn_status zdnn_matmul_bcast_op_ext(const zdnn_ztensor *input_a,
  ...
  if (get_num_procs() == 1)
    status = seq_matmul(
        input_a, input_b, input_c, op_type, output, /*is_bcast=*/true);
  else
    status = omp_matmul(input_a, input_b, input_c, op_type, output,
        /*is_bcast=*/true);
#else
  status =
      seq_matmul(input_a, input_b, input_c, op_type, output, /*is_bcast=*/true);
#endif
...
zdnn_status zdnn_matmul_transpose_op_ext(const zdnn_ztensor *input_a,
  ...
  zdnn_status status = zdnn_matmul_transpose_op(
      input_a, input_b, input_c, transpose_a, transpose_b, opType, output);

  CHECK_ZDNN_STATUS(status, "zdnn_matmul_transpose");

#ifdef ZDNNX_DEBUG
  STOP_TIMING("Transposed MatMul");
#endif
...
zdnn_status zdnn_add_ext(const zdnn_ztensor *inputA, const zdnn_ztensor *inputB,
  ...
  zdnn_status status = call_binary_op(inputA, inputB, output, ZDNNX_ADD_OP);
...
zdnn_status zdnn_sub_ext(const zdnn_ztensor *inputA, const zdnn_ztensor *inputB,
  ...
  zdnn_status status = call_binary_op(inputA, inputB, output, ZDNNX_SUB_OP);
...
zdnn_status zdnn_mul_ext(const zdnn_ztensor *inputA, const zdnn_ztensor *inputB,
  ...
  zdnn_status status = call_binary_op(inputA, inputB, output, ZDNNX_MUL_OP);
...
zdnn_status zdnn_div_ext(const zdnn_ztensor *inputA, const zdnn_ztensor *inputB,
  ...
  zdnn_status status = call_binary_op(inputA, inputB, output, ZDNNX_DIV_OP);
...
zdnn_status zdnn_min_ext(const zdnn_ztensor *inputA, const zdnn_ztensor *inputB,
  ...
  zdnn_status status = call_binary_op(inputA, inputB, output, ZDNNX_MIN_OP);
...
zdnn_status zdnn_max_ext(const zdnn_ztensor *inputA, const zdnn_ztensor *inputB,
  ...
  zdnn_status status = call_binary_op(inputA, inputB, output, ZDNNX_MAX_OP);
...
zdnn_status zdnn_exp_ext(const zdnn_ztensor *input, zdnn_ztensor *output) {
  ...
  zdnn_status status = call_unary_op(input, NULL, output, ZDNNX_EXP_OP);
...
zdnn_status zdnn_log_ext(const zdnn_ztensor *input, zdnn_ztensor *output) {
  ...
  zdnn_status status = call_unary_op(input, NULL, output, ZDNNX_LOG_OP);
...
zdnn_status zdnn_relu_ext(const zdnn_ztensor *input, const void *clipping_value,
  ...
  zdnn_status status =
      call_unary_op(input, clipping_value, output, ZDNNX_RELU_OP);
...
zdnn_status zdnn_sigmoid_ext(const zdnn_ztensor *input, zdnn_ztensor *output) {
  ...
  zdnn_status status = call_unary_op(input, NULL, output, ZDNNX_SIGMOID_OP);
...
zdnn_status zdnn_tanh_ext(const zdnn_ztensor *input, zdnn_ztensor *output) {
  ...
  zdnn_status status = call_unary_op(input, NULL, output, ZDNNX_TANH_OP);
...
#ifdef ZDNNX_WITH_OMP
  if (get_num_procs() == 1)
    status = seq_softmax(input, save_area, act_func, output);
  else
    status = omp_softmax(input, save_area, act_func, output);
#else
  status = seq_softmax(input, save_area, act_func, output);
#endif
...
zdnn_status zdnn_gelu_ext(const zdnn_ztensor *input, zdnn_ztensor *output) {
  ...
  zdnn_status status = call_unary_op(input, NULL, output, ZDNNX_GELU_OP);
...
zdnn_status zdnn_invsqrt_ext(
  ...
  zdnn_status status = call_unary_op(input, &epsilon, output, ZDNNX_INVSQRT_OP);
...
zdnn_status zdnn_sqrt_ext(const zdnn_ztensor *input, zdnn_ztensor *output) {
  ...
  zdnn_status status = call_unary_op(input, NULL, output, ZDNNX_SQRT_OP);
