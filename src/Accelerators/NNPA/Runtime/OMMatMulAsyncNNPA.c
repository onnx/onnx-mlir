...
printf("AAA3\n"); fflush(stdout);
  ...
  printf("emit_zdnn_matmul_op Time: %ld.%06ld sec\n",
      (long int)ttime.tv_sec, (long int)ttime.tv_usec);
  fflush(stdout);
#endif
printf("AAA4\n"); fflush(stdout);
  return NULL;
...
void omTensorMatMulAsync(void *Y, OMTensor *threadTensor, void *A, void *B, void *C) {
  printf("XXXX omTensorMatMulAsync(%p, %p, %p, %p, %p): called\n", Y, threadTensor, A, B, C);fflush(stdout);
  ...
  for (int i = zaiu_cpuid_from[zaiu_id]; i <= zaiu_cpuid_to[zaiu_id]; i++) CPU_SET(i, &cpuset);
