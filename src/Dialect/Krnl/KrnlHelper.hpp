...
void krnl_matmul(
    ...
    // full. Partial tiles (due to computation on the edges of the matrices)
    // are handled differently (using the UBs), so no need to worry about
    // this. Empty means no padding was used.
    ArrayRef<int64_t> aTileSize, ArrayRef<int64_t> bTileSize,
    ArrayRef<int64_t> cTileSize,
    // Optimizations for code gen.
    bool simdize, bool unroll, bool overcompute);
...
// Use _ie suffix below as often the typecheck has issues distinguising
// between Value and IndexExpr calls.
