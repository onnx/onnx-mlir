...
Value reverseWeightTensor(
  ...
  // So, transpose target dimension in advance.
  ...
  // 2. Reverse all elements for spatial dimension
  // 2.1 The first two dimensions can be reversed using two ReverseSequence in
  // the first loop. Reverse D0 and D1 , then D2 and D3 ... for "D0 x D1 x D2 x
  // ... x Dn x N x C"
  for (int i = 0; i < spatialRank / 2; i += 2) {
    // Reverse the first and sconde dimension
    Value reverse0 =
        reverseAllElements(rewriter, loc, transposedInput, /*dimension*/ 0);
    Value reverse1 =
        reverseAllElements(rewriter, loc, reverse0, /*dimension*/ 1);
    // Move reversed two dimensions to the last for next reverse.
  ...
  // 3. Reverse non-spatial dimensions
