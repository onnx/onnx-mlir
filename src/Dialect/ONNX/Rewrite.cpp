...
Value reverseWeightTensor4D(
  ...
  //  SmallVector<int64_t, 4> permsval({2, 0, 1}); // for 3D wieight
  //  SmallVector<int64_t, 4> permsval({2, 3, 0, 1}); // for 4D wieight
  //  SmallVector<int64_t, 4> permsval({2, 3, 4, 0, 1}); // for 5D wieight
  ...
  for (int i = 0; i < permsval.size(); ++i)  
    LLVM_DEBUG(llvm::dbgs() << "permsval " << permsval[i] << "\n");  
  Value transposedInput = emitONNXTranspose(loc, rewriter, input, perms);
  LLVM_DEBUG(llvm::dbgs() << "transposedInput " << transposedInput << "\n");  

  // 2. Create `sequence_lengths`, `batch_axis` and `time_axis` to reverse all elemtns.
  // When reverse the first dim of input(d0 x d1), set `batch_axis` = 1, and `time_axis` = 0 and create [d0, d0,...,d0] as `sequence_lengths` whose the number of elements are d1.
  // Example: input(d0 x d1) = (4 x 3)) then, `sequence_lenghts` is [4, 4, 4].
  // When reverse the second dim of input(d0 x d1), set `batch_axis` = 0, and `time_axis` = 1 and create [d1, d1,...,d1] as `sequence_lengths` whose the number of elements are d0.
  // Example: input(d0 x d1) = (4 x 3)) then, `sequence_lenghts` is [3, 3, 3, 3].
  ShapedType transposedInputType = transposedInput.getType().cast<ShapedType>();
  ArrayRef<int64_t> transposedInputShape = transposedInputType.getShape();
  SmallVector<int64_t, 4> slensTime, slensBatch;
  for (int i = 0; i < transposedInputShape[1]; ++i)  
    slensBatch.emplace_back(transposedInputShape[0]);
  Value reverse0 = emitONNXReverseSequence(
      loc, rewriter, transposedInput, slensBatch, /*batch_axis*/ 1, /*time_axis*/ 0);
  for (int i = 0; i < transposedInputShape[0]; ++i)
    slensTime.emplace_back(transposedInputShape[1]);
  Value reverse1 = emitONNXReverseSequence(loc, rewriter, reverse0,
      slensTime, /*batch_axis*/ 0, /*time_axis*/ 1);
  LLVM_DEBUG(llvm::dbgs() << "revers1 " << reverse1 << "\n");  
  Value reverse2;
  if (spatialRank == 3) {
    ...
    LLVM_DEBUG(llvm::dbgs() << "revers11 " << reverse11 << "\n");
    ShapedType reverse11Type = reverse11.getType().cast<ShapedType>();
    ArrayRef<int64_t> reverse11Shape = reverse11Type.getShape();
    SmallVector<int64_t, 4> slensBatch3d;
    for (int i = 0; i < reverse11Shape[1]; ++i)
      slensBatch3d.emplace_back(reverse11Shape[0]);
     Value reverse12 = emitONNXReverseSequence(loc, rewriter, reverse11,
        slensBatch3d, /*batch_axis*/ 1, /*time_axis*/ 0);
    LLVM_DEBUG(llvm::dbgs() << "revers12 " << reverse12 << "\n");    
   ...
   // Transpose D2xD0xD1xD2x...xNxC to NxCxD0xD1xD2x...
    ...
    LLVM_DEBUG(llvm::dbgs() << "revers2 " << reverse2 << "\n");        
  } else {
    // Transpose in order to reverse other elements in the tensor
