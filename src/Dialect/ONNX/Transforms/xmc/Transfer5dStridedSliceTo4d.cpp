...
static Value createI64Constant(PatternRewriter &rewriter, Location loc,
...
static Value createShapeConstant(PatternRewriter &rewriter, Location loc,
...
findDimsToCollapse(llvm::ArrayRef<int64_t> begins, llvm::ArrayRef<int64_t> ends,
...
struct Transfer5dStridedSliceTo4dPattern : public OpRewritePattern<ONNXSliceOp> {
  ...
  auto quant4DOutputType = RankedTensorType::get(output4DShape, quantElemType);
  ...
  // Step 1: Reshape
  Value inputReshapeQuant4D = rewriter
      .create<ONNXReshapeOp>(loc, quant4DInputType, sliceInput,
          shapeConst4DInput)
      .getResult();
  ...
  Value outputReshapeQuant5D = rewriter
      .create<ONNXReshapeOp>(loc, outputType, sliceQuant4D, shapeConst5DOutput)
      .getResult();
