...
public:
  ...
  LogicalResult matchAndRewrite(ONNXReshapeOp reshapeOut,
    ...
    if (!reshapeOutInputType || reshapeOutInputType.getRank() != 5)
      return rewriter.notifyMatchFailure(reshapeOut,
          "expected 5D input to output reshape");
    ...
    if (!isTransposePerm03412(transposeOp))
      return rewriter.notifyMatchFailure(reshapeOut,
          "transpose perm must be {0, 3, 4, 1, 2}");
    ...
    if (!eltwise0 || !eltwise1)
      return rewriter.notifyMatchFailure(reshapeOut,
          "concat inputs must be from supported elementwise ops");
    ...
    auto eltwise0ElemTy =
        cast<RankedTensorType>(eltwise0->getResult(0).getType()).getElementType();
    auto eltwise1ElemTy =
        cast<RankedTensorType>(eltwise1->getResult(0).getType()).getElementType();
    auto concatElemTy =
        cast<RankedTensorType>(concatOp.getResult().getType()).getElementType();
    auto transposeElemTy =
        cast<RankedTensorType>(transposeOp.getResult().getType()).getElementType();

    // Compute 4D shapes by merging dims 1 and 2: [N, C*D, H, W]
    SmallVector<int64_t, 4> shape0_4D = {shape0_5D[0],
        shape0_5D[1] * shape0_5D[2], shape0_5D[3], shape0_5D[4]};
    SmallVector<int64_t, 4> shape1_4D = {shape1_5D[0],
        shape1_5D[1] * shape1_5D[2], shape1_5D[3], shape1_5D[4]};
    ...
    auto newReshape0 = rewriter.create<ONNXReshapeOp>(loc, reshape0Type4D,
        reshape0.getData(), shape0Const4D);
    auto newReshape1 = rewriter.create<ONNXReshapeOp>(loc, reshape1Type4D,
        reshape1.getData(), shape1Const4D);
    ...
    SmallVector<int64_t, 4> concatShape4D = {shape0_4D[0], shape0_4D[1],
        shape0_4D[2], shape0_4D[3] + shape1_4D[3]};
    auto concatType4D = RankedTensorType::get(concatShape4D, concatElemTy);
    auto newConcat = rewriter.create<ONNXConcatOp>(loc, concatType4D,
        ValueRange{newEltwise0, newEltwise1}, /*axis=*/3);

    // Create new 4D transpose with perm {0, 2, 3, 1}
    SmallVector<int64_t, 4> transposeShape4D = {concatShape4D[0],
        concatShape4D[2], concatShape4D[3], concatShape4D[1]};
    ...
    auto finalReshape = rewriter.create<ONNXReshapeOp>(loc, outputType,
        newTranspose.getResult(), outputShapeConst);
...
class Transfer5dBlockWithConcatPattern : public OpRewritePattern<ONNXReshapeOp> {
public:
  using OpRewritePattern<ONNXReshapeOp>::OpRewritePattern;
  ...
  LogicalResult matchAndRewrite(ONNXReshapeOp reshapeOut,
    ...
    SmallVector<int64_t, 4> shape0_4D = {shape0_5D[0], shape0_5D[1],
        shape0_5D[2], shape0_5D[3] * shape0_5D[4]};
    SmallVector<int64_t, 4> shape1_4D = {shape1_5D[0], shape1_5D[1],
        shape1_5D[2], shape1_5D[3] * shape1_5D[4]};
    ...
    SmallVector<int64_t, 4> concatShape4D = {shape0_4D[0], shape0_4D[1],
        shape0_4D[2] + shape1_4D[2], shape0_4D[3]};
    auto concatType4D = RankedTensorType::get(concatShape4D, concatElemTy);
    auto newConcat = rewriter.create<ONNXConcatOp>(loc, concatType4D,
        ValueRange{newEltwise0, newEltwise1}, /*axis=*/2);
    ...
    auto outputShapeConst = createShapeConstant(rewriter, loc, outputShape);
    auto finalReshape = rewriter.create<ONNXReshapeOp>(loc, outputType,
        newConcat.getResult(), outputShapeConst);
...
public:
  ...
  LogicalResult matchAndRewrite(ONNXReshapeOp reshapeOut,
    ...
    if (!eltwiseOp)
      return rewriter.notifyMatchFailure(reshapeOut,
          "expected supported elementwise op before reshape");
    ...
    if (!transposeOp)
      return rewriter.notifyMatchFailure(reshapeOut,
          "one eltwise input must be from transpose op");
    ...
    auto transposeElemTy =
        cast<RankedTensorType>(transposeOp.getResult().getType()).getElementType();
    auto eltwiseElemTy =
        cast<RankedTensorType>(eltwiseOp->getResult(0).getType()).getElementType();
    ...
    // Create new 4D transpose with perm {0, 2, 3, 1}
    SmallVector<int64_t, 4> transposeShape4D = {shape0_4D[0], shape0_4D[2],
        shape0_4D[3], shape0_4D[1]};
    auto transposeType4D =
        RankedTensorType::get(transposeShape4D, transposeElemTy);
    auto newTranspose = rewriter.create<ONNXTransposeOp>(loc, transposeType4D,
    ...
    Value lhs =
        transposeWasFirstOperand ? newTranspose.getResult() : newReshape1.getResult();
    Value rhs =
        transposeWasFirstOperand ? newReshape1.getResult() : newTranspose.getResult();

    auto lhsType = cast<RankedTensorType>(lhs.getType());
    auto rhsType = cast<RankedTensorType>(rhs.getType());
    SmallVector<int64_t> eltwiseShape;
    if (!OpTrait::util::getBroadcastedShape(lhsType.getShape(),
            rhsType.getShape(), eltwiseShape))
      return failure();
...
struct Transfer5dBlockTo4dPass
    : public PassWrapper<Transfer5dBlockTo4dPass,
