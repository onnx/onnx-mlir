...
bool isNotConstant(Value val) { return !isConstant(val); }
...
struct RemoveReshapeAroundSigmoid : public OpRewritePattern<ONNXReshapeOp> {
  ...
  LogicalResult matchAndRewrite(
      ONNXReshapeOp reshape2, PatternRewriter &rewriter) const override {
    ...
    auto newReshape = rewriter.create<ONNXReshapeOp>(loc, reshape2.getType(),
        newSigmoid.getResult(), reshape2.getShape(),
...
struct RemoveReshapeAroundBinaryOp : public OpRewritePattern<ONNXReshapeOp> {
  ...
  LogicalResult matchAndRewrite(
      ONNXReshapeOp reshape3, PatternRewriter &rewriter) const override {
    // Check if shape is constant
    ...
    auto binaryOutputType =
        mlir::dyn_cast<RankedTensorType>(binaryOp.getType());
    if (!binaryOutputType)
      return failure();
    ...
    auto newReshape = rewriter.create<ONNXReshapeOp>(loc, reshape3.getType(),
        newBinaryOp.getResult(), reshape3.getShape(),
...
struct RemoveRedundantReshapePass
    : public PassWrapper<RemoveRedundantReshapePass,
