...
public:
  ...
  LogicalResult matchAndRewrite(ONNXExpandOp op, OpAdaptor adaptor,
    ...
    auto inputType = llvm::dyn_cast_or_null<RankedTensorType>(adaptor.getInput().getType());
    auto outputType = llvm::dyn_cast_or_null<RankedTensorType>(op.getResult().getType());
    ...
    // If inputRank is inferior to shapeRank we need to introduce a reshape before the tile
    auto newInput = adaptor.getInput();
    if (inputRank < shapeArray.size()) {
      llvm::SmallVector<int64_t> newShape = getNewShape(inputType.getShape(), outputType.getShape());
      TosaBuilder tosaBuilder(rewriter, op->getLoc());
      newInput = tosaBuilder.reshape(adaptor.getInput(), newShape);
    }

    auto denseShape = getMultiplies(op, cast<RankedTensorType>(newInput.getType()).getShape(), shapeArray);
...
// Supported element types for tosa.tile
  ...
  static llvm::SmallVector<int64_t> getNewShape(const llvm::ArrayRef<int64_t> &inputShape, const llvm::ArrayRef<int64_t> &outputShape) {
    llvm::SmallVector<int64_t> result;
    size_t inputIdx = 0;
    for (auto outputDimension : outputShape) {
      // - If the inputIdx goes beyond the input shape, it means we are extending the shape: Ex: 1x3x4 -> 1x3x4x1
      // - If the input dim is < output dim and do not divide it, it's a dimension being added: Ex: 3x1 -> 2x1x6 (first dim is a new dim and not a tiled original one)
      // - If the output dim is < input dim, it's also a dim being added: Ex: 2x3x4 -> 1x2x3x4
      if (inputIdx >= inputShape.size() || 
         (inputShape[inputIdx] < outputDimension && outputDimension % inputShape[inputIdx] != 0) || 
  ...
  static DenseI64ArrayAttr getMultiplies(ONNXExpandOp &op, const llvm::ArrayRef<int64_t> &inputShape, const llvm::ArrayRef<int64_t> &shapeAttr) {
