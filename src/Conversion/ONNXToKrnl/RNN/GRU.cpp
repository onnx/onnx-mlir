...
void calculateState<GruState, GruActivationPack, GruWeightPack, GruBiasPack>(
  ...
  Value XtWT =
      create.onnx.matmulToMemref(matrixAllGatesType, Xt, weightPack.WT);
  ...
  if (state.linearBeforeReset) {
    ...
    Value HtRT =
        create.onnx.matmulToMemref(matrixAllGatesType, Ht, weightPack.RT);
    ...
    Value HtRz = create.onnx.matmulToMemref(matrixType, Ht, weightPack.Rz);
    Value HtRr = create.onnx.matmulToMemref(matrixType, Ht, weightPack.Rr);
    Value rt, rtHt;
    ...
    // Emit (rt (.) Ht-1)*(Rh^T)
    Value rtHtRh = create.onnx.matmulToMemref(matrixType, rtHt, weightPack.Rh);
