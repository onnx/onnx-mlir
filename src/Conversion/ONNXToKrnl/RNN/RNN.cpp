...
getWeightPack<ONNXRNNOp, RnnWeightPack>(
  ...
  if (direction == FORWARD || direction == BIDIRECTIONAL) {
    weightForward.Wi =
        emitTranspose(rewriter, loc, wTranspose2DTy, fW, permAttr);
    weightForward.Ri =
        emitTranspose(rewriter, loc, rTranspose2DTy, fR, permAttr);
  }
  if (direction == REVERSE || direction == BIDIRECTIONAL) {
    weightReverse.Wi =
        emitTranspose(rewriter, loc, wTranspose2DTy, bW, permAttr);
    weightReverse.Ri =
        emitTranspose(rewriter, loc, rTranspose2DTy, bR, permAttr);
  }
