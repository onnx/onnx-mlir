  ...
  Value posInf = create.math.positiveInf(elementType);
     ConversionPatternRewriter &rewriter, Location loc, Type elemType) {
       ...
       double detectNegAttribute =
           dyn_cast<ONNXIsInfOp>(op).getDetectNegative();
       double detectPosAttribute =
           dyn_cast<ONNXIsInfOp>(op).getDetectPositive();
     ...
     template <>
     Value emitScalarOpFor<ONNXHardSigmoidOp>(
       ...
       Value operand = scalarOperands[0];
       double alphaLit =
           dyn_cast<ONNXHardSigmoidOp>(op).getAlpha().convertToFloat();
       double betaLit =
           dyn_cast<ONNXHardSigmoidOp>(op).getBeta().convertToFloat();
       // Create constants.
       MultiDialectBuilder<MathBuilder> create(rewriter, loc);
       Value zero = create.math.constant(elementType, 0);
       Value one = create.math.constant(elementType, 1);
     ...
     Value emitScalarOpFor<ONNXEluOp>(ConversionPatternRewriter & rewriter,
       ...
       return create.math.select(lessThanZero,
           create.math.mul(alpha, create.math.sub(exp, one)), operand);
     }
     ...
     double analyzeSimdFor<ONNXReluOp>(Type t, int64_t & von, int64_t & son) {
       return simdAnalysis({GenericOps::CompareGop, GenericOps::SelectGop},
           {1, 1}, t, von, son);
     }
     ...
     template <>
     Value emitScalarOpFor<ONNXLeakyReluOp>(
         ConversionPatternRewriter & rewriter, Location loc, Operation * op,
         Type elementType, ArrayRef<Value> scalarOperands) {
       ...
       Value operand = scalarOperands[0];
       double alphaLit =
           dyn_cast<ONNXLeakyReluOp>(op).getAlpha().convertToFloat();
       MultiDialectBuilder<MathBuilder> create(rewriter, loc);
       Value zero = create.math.constant(elementType, 0);
     ...
     template <>
     Value emitScalarOpFor<ONNXReciprocalOp>(
         ConversionPatternRewriter & rewriter, Location loc, Operation * op,
         Type elementType, ArrayRef<Value> scalarOperands) {
     ...
     template <>
     Value emitScalarOpFor<ONNXLessOrEqualOp>(
         ConversionPatternRewriter & rewriter, Location loc, Operation * op,
         Type elementType, ArrayRef<Value> scalarOperands) {
     ...
     template <>
     Value emitScalarOpFor<ONNXGreaterOrEqualOp>(
         ConversionPatternRewriter & rewriter, Location loc, Operation * op,
         Type elementType, ArrayRef<Value> scalarOperands) {
    ...
    return rem;
       ...
       if (create.math.isIntegerWithVector(elementType)) {
         // TODO: implement
         llvm_unreachable(
             "not support integers at this moment since MLIR integers "
             "are signless.");
       }
       llvm_unreachable("unsupported element type");
     }
     ...
     double analyzeSimdFor<ONNXMeanOp>(Type t, int64_t & von, int64_t & son) {
       return simdAnalysis({GenericOps::ArithmeticGop, GenericOps::DivGop},
           {1, 1}, t, von, son);
     }

     template <>
     Value emitPostProcessingFor<ONNXMeanOp>(
         ConversionPatternRewriter & rewriter, Location loc, Operation * op,
         Type elementType, Value scalarResult) {
       MultiDialectBuilder<MathBuilder> create(rewriter, loc);
     ...
     Value emitScalarOpFor<ONNXRoundOp>(ConversionPatternRewriter & rewriter,
       ...
       Value y1 =
           create.math.select(rGreaterThanHalf, create.math.add(y, one), y);
     ...
     int64_t canBeVectorized(ShapeHelperType & shapeHelper, MDBuilder & create,
       ...
       if (vectorizedOpNum >= vrNum / 2)
         simdUnroll =
             1; // TODO, it would appear to be beneficial to always have 2.
       ...
       if (collapsedInnermostLoops > 0 &&
         ...
         // We have a partially flattened operator. Since we do only simdize
         // entire loops (i.e. we don't support scalar epilogues at this time),
         // make sure the static size is a multiple of the VL. Get the VL of the
         // store (output's element type).
         ...
         for (int64_t u = simdUnroll; u > 0; --u) {
           if (collapsedLiteralSize % (u * VL) == 0) {
             LLVM_DEBUG(llvm::dbgs()
                        << "  partial flattened dims "
                        << collapsedInnermostLoops << " with size "
                        << collapsedLiteralSize << " works with VL " << VL
                        << " and unroll " << u << "\n");
             simdUnroll = u;
             gotOne = true;
             break;
     ...
     static LogicalResult getFullyFlattenedSimdCode(
       ...
       Value alloc = create.mem.alignedAllocWithSimdPadding(outputMemRefType,
           shapeHelper->getOutputDims(), simdUnroll, alignment);
       // Create flat inputs.
           ...
           [&](KrnlBuilder &ck, ValueRange loopInd) {
             ...
             for (Value flatOper : flatOperands) {
               ...
               MemRefType memRefType =
                   flatOper.getType().dyn_cast<MemRefType>();
             ...
             if (isUnaryOp) {
               ...
               for (unsigned i = 1; i < numArgs; ++i) {
                 ...
                 // Fold.
                 accumulated = emitScalarOpFor<OP_TYPE>(rewriter,
     ...
     static LogicalResult getPartiallyFlattenedSimdCode(
         ...
         MemRefType outputMemRefType, ValueRange operands, int64_t alignment,
         int64_t simdUnroll, int64_t collapsedInnermostLoops,
         bool ruledOutBroadcast, bool isUnaryOp) {
       Type outputElementType = outputMemRefType.getElementType();
       unsigned numArgs = op->getNumOperands();
       ...
       int64_t VL =
           create.vec.getMachineVectorLength(outputElementType) * simdUnroll;
       // Alloc memory with padding for SIMD.
       // For the moment, its ok to go here; if we truly have partial flattening
       // of the simd code, then we only do it with static memref size that are
       // multiples of VL * simdUnroll, so there should be no padding anyway.
       // This will change if we do partial flattening with non-multiple of VL *
       // simdUnroll.
       Value alloc = create.mem.alignedAllocWithSimdPadding(outputMemRefType,
           shapeHelper->getOutputDims(), simdUnroll, alignment);
       ...
       // Create loop iteration (flattened to output dim - inner dim + 1) with
       // inner one and blocked by mVL.
       int64_t rank = outputDims.size() - collapsedInnermostLoops + 1;
       LLVM_DEBUG(llvm::dbgs()
                  << "SIMD partial flatten with loop rank " << rank << "\n");
             ...
             for (int64_t i = 0; i < (int64_t)flatOperands.size(); ++i) {
               ...
               if (hasOneElementInInnermostDims(flatOper, 1)) {
                 ...
                 if (hasOneElement(flatOper)) {
                   ...
                   LogicalResult res = shapeHelper->getAccessExprs(flatOper, i,
                       outputAccessExprs, scalarAccessFct, /*flattened*/ true,
                       ruledOutBroadcast);
                   assert(succeeded(res) && "Could not compute access indices");
                 }
                 Value loadedVal =
                     create.krnl.loadIE(flatOper, scalarAccessFct);
                 Value splatValue = create.vec.splat(vecType, loadedVal);
                 loadedVals.emplace_back(splatValue);
               } else {
     ...
     typedef mlir::Value (*EmitScalarFunc)(
     ...
     template <typename T>
     bool enqueueFusedOpImpl(Operation * op,
     ...
     bool enqueueFusedOp(Operation * op,
         SmallVector<Operation *, 2> & fusibleOps,
         SmallVector<EmitScalarFunc, 2> & fuseEmitFunctions);
     ...
     public:
       ...
       OpFusionHelper(
           ...
           : op_(startOp), rewriter_(rewriter), fusibleOps_(),
             fuseEmitFunctions_() {}
       ...
       bool checkFusedOp(Operation *op, SmallVector<Operation *, 2> &fusibleOps,
         ...
         return enqueueFusedOp<mlir::ONNXAbsOp, mlir::ONNXAtanOp,
             mlir::ONNXCastOp, mlir::ONNXCeilOp, mlir::ONNXCosOp,
             mlir::ONNXCoshOp, mlir::ONNXDequantizeLinearOp, mlir::ONNXEluOp,
             mlir::ONNXErfOp, mlir::ONNXAcosOp, mlir::ONNXAcoshOp,
             mlir::ONNXAsinOp, mlir::ONNXAsinhOp, mlir::ONNXAtanhOp,
             mlir::ONNXExpOp, mlir::ONNXFloorOp, mlir::ONNXHardSigmoidOp,
             ...
             mlir::ONNXSeluOp, mlir::ONNXSigmoidOp, mlir::ONNXSignOp,
             mlir::ONNXSinOp, mlir::ONNXSinhOp, mlir::ONNXSoftplusOp,
             mlir::ONNXSoftsignOp, mlir::ONNXSqrtOp, mlir::ONNXTanOp,
             mlir::ONNXTanhOp>(op, fusibleOps, fuseEmitFunctions);
       }
     ...
     struct ONNXElementwiseUnaryOpLowering
       ...
       ONNXElementwiseUnaryOpLowering(TypeConverter &typeConverter,
       ...
       LogicalResult matchAndRewrite(ElementwiseUnaryOp elmsOp,
         ...
         // Just call scalar computation and return the result. This is
         // efficient when elementwise ops are used as activations for ops like
         // LSTM/GRU/RNN.
         ...
         Type convertedType =
             this->typeConverter->convertType(outputTensorType);
         int64_t alignment =
         ...
         if (enableSIMD && !isScalar && !hasNonIdentityLayout(operands)) {
           ...
           if (simdUnroll > 0)
             return getFullyFlattenedSimdCode<ElementwiseUnaryOp>(rewriter,
                 create, &shapeHelper, op, outputMemRefType, operands,
     ...
     // This template can be used for binary ops that return a result whose type
     ...
     struct ONNXElementwiseBinaryOpLowering
       ...
       LogicalResult matchAndRewrite(ElementwiseBinaryOp elmsOp,
           OpAdaptor adaptor, ConversionPatternRewriter &rewriter) const final {
         Operation *op = elmsOp.getOperation();
         ValueRange operands = adaptor.getOperands();
         ...
         bool hasManageableBroadcast =
             shapeHelper.hasManageableBroadcastForInnerDims(
                 collapsedInnermostLoops, collapsedLiteralSize,
                 collapsedDynamicSize, dimAnalysis);
         LLVM_DEBUG({
         ...
         if (enableSIMD && !isScalar && hasManageableBroadcast &&
           ...
           if (simdUnroll > 0)
             return getPartiallyFlattenedSimdCode<ElementwiseBinaryOp>(rewriter,
                 create, &shapeHelper, op, outputMemRefType, operands,
                 alignment, simdUnroll, collapsedInnermostLoops, hasNoBroadcast,
                 /*unary*/ false);
         }
         LLVM_DEBUG(llvm::dbgs() << "  scalar execution\n");
           ...
           create.krnl.iterateIE(loopDef, loopDef, lbs, ubs,
               [&](KrnlBuilder &createKrnl, ValueRange loopInd) {
                 ...
                 LogicalResult res = shapeHelper.getAccessExprs(operands[0], 0,
                     outputAccessExprs, lhsAccessExprs,
                 ...
                 res = shapeHelper.getAccessExprs(operands[1], 1,
                     outputAccessExprs, rhsAccessExprs,
                     /*flattened dims*/ false, hasNoBroadcast);
                 assert(succeeded(res) && "Could not compute access indices");
     ...
     struct ONNXElementwiseVariadicOpLowering
       ...
       LogicalResult matchAndRewrite(ElementwiseVariadicOp elmsOp,
           OpAdaptor adaptor, ConversionPatternRewriter &rewriter) const final {
         Operation *op = elmsOp.getOperation();
         ...
         if (enableSIMD && !isScalar && hasManageableBroadcast &&
             !hasNonIdentityLayout(operands)) {
           // SIMD is enabled for this operation, test if desired and feasible
           int64_t simdUnroll = canBeVectorized<ONNXBroadcastOpShapeHelper,
               ElementwiseVariadicOp>(shapeHelper, create, outputMemRefType,
               collapsedInnermostLoops, collapsedLiteralSize);
           if (simdUnroll > 0)
             return getPartiallyFlattenedSimdCode<ElementwiseVariadicOp>(
           ...
           create.krnl.iterateIE(loopDef, loopDef, lbs, ubs,
               [&](KrnlBuilder &createKrnl, ValueRange loopInd) {
                 ...
                 for (unsigned i = 1; i < numArgs; ++i) {
                   ...
                   SmallVector<IndexExpr, 4> oprdAccessExprs;
                   LogicalResult res = shapeHelper.getAccessExprs(operands[i],
                       i, outputAccessExprs, oprdAccessExprs,
                       /*flattened dims*/ false, hasNoBroadcast);
                   assert(succeeded(res) && "Could not compute access indices");
                   Value next = createKrnl.loadIE(operands[i], oprdAccessExprs);
                   // Fold.
                   accumulated =
                       emitScalarOpFor<ElementwiseVariadicOp>(rewriter, loc, op,
                           outputElementType, {accumulated, next});
                 }

                 Value finalResult =
                     emitPostProcessingFor<ElementwiseVariadicOp>(
     ...
     struct ONNXWhereOpLowering : public ConversionPattern {
       ...
       LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
         ...
         // Shape helper.
         MultiDialectBuilder<IndexExprBuilderForKrnl, KrnlBuilder,
             MemRefBuilder>
             create(rewriter, loc);
         ONNXBroadcastOpShapeHelper shapeHelper(op, operands, &create.krnlIE);
         shapeHelper.computeShapeAndAssertOnFailure();
