  ...
  LogicalResult matchAndRewrite(ONNXDetOp detOp, OpAdaptor adaptor,
    ...
    if (!mlir::isa<FloatType>(elementType))
      return op->emitOpError(
          "Det lowering only supports floating point element types");
    ...
    Value rowDim = create.krnlIE.getShapeAsDim(X, xRank - 2).getValue();
    Value colDim = create.krnlIE.getShapeAsDim(X, xRank - 1).getValue();
    Value M = create.math.min(rowDim, colDim);
