//===- KrnlParallelFor.cpp - Implementation of Async Parallel For --------===//
//
...
//   }
//
struct KrnlParallelForPass
    : public PassWrapper<KrnlParallelForPass, OperationPass<ModuleOp>> {
  KrnlParallelForPass() = default;

  KrnlParallelForPass(int32_t numWorkerThreads,
                       int32_t minTaskSize) {
    this->numWorkerThreads = numWorkerThreads;
    this->minTaskSize = minTaskSize;
  }
  int32_t numWorkerThreads, minTaskSize;
  void runOnOperation() override;
};

struct KrnlParallelForRewrite : public OpRewritePattern<scf::ParallelOp> {
public:
  KrnlParallelForRewrite(
      MLIRContext *ctx,  int32_t numWorkerThreads,
...
LogicalResult
KrnlParallelForRewrite::matchAndRewrite(scf::ParallelOp op,
  ...
  auto dispatch = [&](OpBuilder &nestedBuilder, Location loc) {
    ...
    // Dispatch parallel compute function by iteratively submitting compute task.
    auto doDispatch = doSequentialDispatch;
...
void KrnlParallelForPass::runOnOperation() {
  MLIRContext *ctx = &getContext();

  RewritePatternSet patterns(ctx);
  populateKrnlParallelForPatterns(
      patterns, numWorkerThreads,
...
std::unique_ptr<Pass> mlir::createKrnlParallelForPass() {
  return std::make_unique<KrnlParallelForPass>();
}

std::unique_ptr<Pass> mlir::createKrnlParallelForPass(
                                                       int32_t numWorkerThreads,
                                                       int32_t minTaskSize) {
  return std::make_unique<KrnlParallelForPass>(numWorkerThreads,
                                                minTaskSize);
}

void mlir::async::populateKrnlParallelForPatterns(
    RewritePatternSet &patterns, int32_t numWorkerThreads,
    const AsyncMinTaskSizeComputationFunction &computeMinTaskSize) {
  MLIRContext *ctx = patterns.getContext();
  patterns.add<KrnlParallelForRewrite>(ctx, numWorkerThreads,
                                        computeMinTaskSize);
}
