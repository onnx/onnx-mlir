...
KrnlParallelDecision decideKrnlParallel(ArrayRef<IndexExpr> lbs,
  ...
  // STEP 0: a single level, today's rules exactly. The only path taken while no
  // site is collapse-eligible (collapseLastExclusiveDim == -1), which is what
  // keeps the flag-off IR bit-identical.
  //
  ...
  // bit-identity requires; the collapse-aware steps use a properly filtered set
  // of levels instead.
