...
public:
  ...
  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    StringRef nodeName = rewriter.getStringAttr(nodeNameStr).strref();

#elif 1
    // Normally the names should always be defined when creating
    // KrnlInstrumentOp, so we should not have to invoke the
    // getExtendedNodeName as it should be simply the name in the NodeName
    // Attribute.
    StringAttr nodeNameAttr = krnl::getExtendedNodeName(
        rewriter, loc, instrumentOp.getNodeNameAttr());
    StringRef nodeName = nodeNameAttr.strref();
#else
