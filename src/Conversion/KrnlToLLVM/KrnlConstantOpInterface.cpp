...
//===------- KrnlConstantOpInterface.cpp - Lower ConstantOpInterface-------===//
//
...
class KrnlConstantOpInterfaceLowering
    : public OpInterfaceConversionPattern<ConstantOpInterface> {

public:
  explicit KrnlConstantOpInterfaceLowering(
...
private:
  ...
  LLVM::GlobalOp lowerDenseResourceConstant(ConstantOpInterface &krnlGlobalOp,
  ...
  LLVM::GlobalOp lowerDenseConstant(ConstantOpInterface &krnlGlobalOp,
      Type globalType, ConversionPatternRewriter &rewriter) const {
    assert(krnlGlobalOp.getValue().has_value() &&
           "Expecting KrnlGlobalOp with a valid value");
  ...
  LLVM::GlobalOp lowerGlobalOpWithExternalFiles(
      ConstantOpInterface &krnlGlobalOp,
  ...
  uint64_t computeSizeInBytes(ConstantOpInterface &krnlGlobalOp) const {
  ...
  LLVM::GlobalOp lowerStringLiteral(ConstantOpInterface &krnlGlobalOp,
...
void populateLoweringKrnlConstantOpInterfacePattern(
    ...
    MLIRContext *ctx) {
  patterns.insert<KrnlConstantOpInterfaceLowering>(typeConverter, ctx);
}
