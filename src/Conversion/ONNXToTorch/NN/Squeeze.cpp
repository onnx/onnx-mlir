...
std::vector<int64_t> getResultShape(
    int dim_index, std::vector<int64_t> tensorShape) {
...
//
// Operands :
//   data       tensor of arbitrary sign and type
//   axes       (attr/operand) tensor of 64-bit signless integer values
//
// Result   :
//   output  (tensor)
//
...
struct ONNXToTorchSqueezeOpLowering : public ConversionPattern {
  ONNXToTorchSqueezeOpLowering(TypeConverter &typeConverter, MLIRContext *ctx)
      : ConversionPattern(
            typeConverter, SqueezeOp::getOperationName(), 1, ctx) {}
...
void populateLoweringONNXToTorchSqueezeOpPattern(RewritePatternSet &patterns,
  ...
  patterns.insert<ONNXToTorchSqueezeOpLowering<ONNXSqueezeV11Op>,
      ONNXToTorchSqueezeOpLowering<ONNXSqueezeOp>>(typeConverter, ctx);
}
