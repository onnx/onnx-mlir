...
struct ONNXGemmOpToTorchLowering : public ConversionPattern {
  ...
  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    auto aTy      = Torch::ValueTensorType::get(context, a_tensor_type.getShape(),
                    a_tensor_type.getElementType());
    auto bTy      = Torch::ValueTensorType::get(context, b_tensor_type.getShape(),
                    b_tensor_type.getElementType());
    auto cTy      = Torch::ValueTensorType::get(context, c_tensor_type.getShape(),
                    c_tensor_type.getElementType());

    auto att  = rewriter.create<torch::TorchConversion::FromBuiltinTensorOp>( loc, aTy, a);
    auto btt  = rewriter.create<torch::TorchConversion::FromBuiltinTensorOp>( loc, bTy, b);
    auto ctt  = rewriter.create<torch::TorchConversion::FromBuiltinTensorOp>( loc, cTy, c);
    ...
    if (transA)
      transposeAVal = rewriter.create<AtenTransposeIntOp>(loc, resultTy, att, f0v, f1v);
    else
      transposeAVal = att;
    llvm::outs() << "\n transposeAVal : " << "\n" << transposeAVal << "\n" << "\n";

    if (transB)
      transposeBVal = rewriter.create<AtenTransposeIntOp>(loc, resultTy, btt, f0v, f1v);
    else
      transposeBVal = btt;
    ...
    if (alpha)
      alphaMulResult = rewriter.create<AtenMulScalarOp>(loc, resultTy, transposeAVal, alpha3v);
    llvm::outs() << "alphaMulResult Value" << "\n" << alphaMulResult << "\n" << "\n";

    if (beta)
      betaMulResult  = rewriter.create<AtenMulScalarOp>(loc, resultTy, ctt, beta3v);
    ...
    Value bmmValue;
    if (alphaMulResult)
      bmmValue = rewriter.create<AtenBmmOp>(loc, resultTy, alphaMulResult, transposeBVal);
    ...
    Value addValue;
    if (bmmValue)
      addValue = rewriter.create<AtenSumOp>(loc, resultTy, bmmValue, betaMulResult);
