 ...
 * Validation 
 * ----------
 * /scripts/docker/build_with_docker.py --external-build --build-dir build --command "build/Ubuntu1804-Release/third-party/onnx-mlir/Release/bin/onnx-mlir --EmitONNXIR --debug --run-torch-pass third-party/onnx-mlir/third_party/onnx/onnx/backend/test/data/node/test_constant/model.onnx"
 * 
 * Limitations
 * -----------
 * uses literal.
  ...
  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    if (op_tensor_type) {
      if (auto integerType = op_tensor_type.getElementType().dyn_cast<IntegerType>()) {
        element_type = IntegerType::get(context, integerType.getWidth(), IntegerType::Signed);
        auto dense_value_attr = value_attr.dyn_cast<::mlir::DenseElementsAttr>();
        ShapedType dense_value_type = RankedTensorType::get(op_tensor_type.getShape(), element_type);
  	...
  	intValues.push_back(n);
      ...
      else if (auto floatType = op_tensor_type.getElementType().dyn_cast<::mlir::FloatType>()) {
        ...
        auto new_dense_value_attr = DenseElementsAttr::get(dense_value_type, floatValues);
        value_attr_finalized = new_dense_value_attr;
      }
      else if (auto stringType = op_tensor_type.getElementType().dyn_cast<::mlir::StringType>()) {
        ...
        auto new_dense_value_attr = DenseElementsAttr::get(dense_value_type, stringValues);
        value_attr_finalized = new_dense_value_attr;
      }
      else {
	...
	element_type = ::mlir::IntegerType::get(context, intType.getWidth(), IntegerType::Signed);
	value_attr_finalized = value_attr;	
      }
    ...
    auto resultTy = Torch::ValueTensorType::get(op1.getContext(), op_tensor_type.getShape(), element_type);
