// RUN: cfg_file=$(dirname %s)/nnpa-cfg.json && save_cfg_file=$(dirname %s)/save-nnpa-cfg.json && onnx-mlir --EmitONNXIR --march=z17 --maccel=NNPA --config-file=$cfg_file --save-config-file=$save_cfg_file --printIR %s && cat $save_cfg_file | FileCheck %s && rm $save_cfg_file
