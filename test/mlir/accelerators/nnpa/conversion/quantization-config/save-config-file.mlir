// RUN: onnx-mlir-opt --nnpa-quant-ops-selection=save-config-file="%S/save-cfg.json load-config-file=%S/cfg.json" --march=z17 --maccel=NNPA --split-input-file %s && cat %S/save-cfg.json | FileCheck %s && rm %S/save-cfg.json
