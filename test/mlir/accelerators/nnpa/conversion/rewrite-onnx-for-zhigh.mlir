...
// CHECK-DAG:       [[VAR_1_:%.+]] = "onnx.Im2Col"([[PARAM_0_]]) <{auto_pad = "NOTSET", kernel_shape = [2, 2], pads = [0, 0, 2, 2]}> : (tensor<5x3x32x32xf32>) -> tensor<?x12x?xf32>
// CHECK-DAG:       [[VAR_2_:%.+]] = onnx.Constant dense<[1024, 12]> : tensor<2xi64>
// CHECK:           [[VAR_3_:%.+]] = "onnx.Reshape"([[PARAM_1_]], [[VAR_2_]]) <{allowzero = 0 : si64}> : (tensor<1024x3x2x2xf32>, tensor<2xi64>) -> tensor<1024x12xf32>
// CHECK-DAG:       [[VAR_4_:%.+]] = "onnx.MatMul"([[VAR_3_]], [[VAR_1_]]) : (tensor<1024x12xf32>, tensor<?x12x?xf32>) -> tensor<?x1024x?xf32>
// CHECK-DAG:       [[VAR_5_:%.+]] = "onnx.Dim"([[PARAM_0_]]) <{axis = 0 : si64}> : (tensor<5x3x32x32xf32>) -> tensor<1xi64>
// CHECK-DAG:       [[VAR_6_:%.+]] = "onnx.Dim"([[PARAM_1_]]) <{axis = 0 : si64}> : (tensor<1024x3x2x2xf32>) -> tensor<1xi64>
// CHECK-DAG:       [[VAR_7_:%.+]] = "onnx.Dim"([[PARAM_0_]]) <{axis = 2 : si64}> : (tensor<5x3x32x32xf32>) -> tensor<1xi64>
// CHECK-DAG:       [[VAR_8_:%.+]] = "onnx.Dim"([[PARAM_0_]]) <{axis = 3 : si64}> : (tensor<5x3x32x32xf32>) -> tensor<1xi64>
...
// CHECK:           [[VAR_13_:%.+]] = "onnx.Add"([[VAR_7_]], [[VAR_9_]]) : (tensor<1xi64>, tensor<1xi64>) -> tensor<1xi64>
// CHECK-DAG:       [[VAR_14_:%.+]] = "onnx.Div"([[VAR_13_]], [[VAR_11_]]) : (tensor<1xi64>, tensor<1xi64>) -> tensor<1xi64>
// CHECK-DAG:       [[VAR_15_:%.+]] = "onnx.Add"([[VAR_8_]], [[VAR_10_]]) : (tensor<1xi64>, tensor<1xi64>) -> tensor<1xi64>
// CHECK:           [[VAR_16_:%.+]] = "onnx.Div"([[VAR_15_]], [[VAR_12_]]) : (tensor<1xi64>, tensor<1xi64>) -> tensor<1xi64>
// CHECK:           [[VAR_17_:%.+]] = "onnx.Concat"([[VAR_5_]], [[VAR_6_]], [[VAR_14_]], [[VAR_16_]]) <{axis = 0 : si64}> : (tensor<1xi64>, tensor<1xi64>, tensor<1xi64>, tensor<1xi64>) -> tensor<4xi64>
// CHECK:           [[VAR_18_:%.+]] = "onnx.Reshape"([[VAR_4_]], [[VAR_17_]]) <{allowzero = 0 : si64}> : (tensor<?x1024x?xf32>, tensor<4xi64>) -> tensor<5x1024x33x33xf32>
// CHECK:           return [[VAR_18_]] : tensor<5x1024x33x33xf32>
// CHECK:         }
// -----
...
// CHECK-DAG:       [[VAR_0_:%.+]] = "onnx.Im2Col"([[PARAM_0_]]) <{auto_pad = "NOTSET", kernel_shape = [7, 7], pads = [3, 3, 3, 3], strides = [2, 2]}> : (tensor<1x3x224x224xf32>) -> tensor<?x147x?xf32>
// CHECK-DAG:       [[VAR_1_:%.+]] = onnx.Constant dense<[64, 147]> : tensor<2xi64>
// CHECK:           [[VAR_2_:%.+]] = "onnx.Reshape"([[PARAM_1_]], [[VAR_1_]]) <{allowzero = 0 : si64}> : (tensor<64x3x7x7xf32>, tensor<2xi64>) -> tensor<64x147xf32>
// CHECK-DAG:       [[VAR_3_:%.+]] = "onnx.MatMul"([[VAR_2_]], [[VAR_0_]]) : (tensor<64x147xf32>, tensor<?x147x?xf32>) -> tensor<?x64x?xf32>
// CHECK-DAG:       [[VAR_4_:%.+]] = onnx.Constant dense<1> : tensor<1xi64>
// CHECK:           [[VAR_5_:%.+]] = "onnx.Unsqueeze"([[PARAM_2_]], [[VAR_4_]]) : (tensor<64xf32>, tensor<1xi64>) -> tensor<64x1xf32>
// CHECK-DAG:       [[VAR_6_:%.+]] = "onnx.Add"([[VAR_3_]], [[VAR_5_]]) : (tensor<?x64x?xf32>, tensor<64x1xf32>) -> tensor<?x64x?xf32>
// CHECK-DAG:       [[VAR_7_:%.+]] = "onnx.Dim"([[PARAM_0_]]) <{axis = 0 : si64}> : (tensor<1x3x224x224xf32>) -> tensor<1xi64>
// CHECK-DAG:       [[VAR_8_:%.+]] = "onnx.Dim"([[PARAM_1_]]) <{axis = 0 : si64}> : (tensor<64x3x7x7xf32>) -> tensor<1xi64>
// CHECK-DAG:       [[VAR_9_:%.+]] = "onnx.Dim"([[PARAM_0_]]) <{axis = 2 : si64}> : (tensor<1x3x224x224xf32>) -> tensor<1xi64>
// CHECK-DAG:       [[VAR_10_:%.+]] = "onnx.Dim"([[PARAM_0_]]) <{axis = 3 : si64}> : (tensor<1x3x224x224xf32>) -> tensor<1xi64>
// CHECK-DAG:       [[VAR_11_:%.+]] = onnx.Constant dense<1> : tensor<1xi64>
// CHECK-DAG:       [[VAR_12_:%.+]] = onnx.Constant dense<1> : tensor<1xi64>
// CHECK-DAG:       [[VAR_13_:%.+]] = onnx.Constant dense<2> : tensor<1xi64>
// CHECK-DAG:       [[VAR_14_:%.+]] = onnx.Constant dense<2> : tensor<1xi64>
// CHECK:           [[VAR_15_:%.+]] = "onnx.Add"([[VAR_9_]], [[VAR_11_]]) : (tensor<1xi64>, tensor<1xi64>) -> tensor<1xi64>
// CHECK-DAG:       [[VAR_16_:%.+]] = "onnx.Div"([[VAR_15_]], [[VAR_13_]]) : (tensor<1xi64>, tensor<1xi64>) -> tensor<1xi64>
// CHECK-DAG:       [[VAR_17_:%.+]] = "onnx.Add"([[VAR_10_]], [[VAR_12_]]) : (tensor<1xi64>, tensor<1xi64>) -> tensor<1xi64>
// CHECK:           [[VAR_18_:%.+]] = "onnx.Div"([[VAR_17_]], [[VAR_14_]]) : (tensor<1xi64>, tensor<1xi64>) -> tensor<1xi64>
// CHECK:           [[VAR_19_:%.+]] = "onnx.Concat"([[VAR_7_]], [[VAR_8_]], [[VAR_16_]], [[VAR_18_]]) <{axis = 0 : si64}> : (tensor<1xi64>, tensor<1xi64>, tensor<1xi64>, tensor<1xi64>) -> tensor<4xi64>
// CHECK:           [[VAR_20_:%.+]] = "onnx.Reshape"([[VAR_6_]], [[VAR_19_]]) <{allowzero = 0 : si64}> : (tensor<?x64x?xf32>, tensor<4xi64>) -> tensor<1x64x112x112xf32>
// CHECK:           return [[VAR_20_]] : tensor<1x64x112x112xf32>
// CHECK:         }
// -----
