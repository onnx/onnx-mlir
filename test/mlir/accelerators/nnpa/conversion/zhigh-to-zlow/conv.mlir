// RUN: onnx-mlir-opt --maccel=NNPA --maccel=NNPA --shape-inference --convert-onnx-to-krnl --canonicalize %s -split-input-file | FileCheck %s 
...
// CHECK-SAME:   ([[PARAM_0_:%.+]]: memref<1x32x32x3xf16, #map0>, [[PARAM_1_:%.+]]: memref<2x2x3x1xf16, #map1>, [[PARAM_2_:%.+]]: memref<1xf16, #map2>) -> memref<1x31x31x1xf16, #map0> {
...
// CHECK-DAG:       [[RES_:%.+]] = memref.alloc() {{.*}}: memref<1x31x31x1xf16, #map0>
...
// CHECK:           "zlow.conv2d"([[PARAM_0_]], [[PARAM_1_]], [[PARAM_2_]], [[RES_1_]], [[RES_]]) {act_func = "ACT_NONE", kernel_shape = [2, 2], padding_type = "VALID_PADDING", strides = [1, 1]} : (memref<1x32x32x3xf16, #map0>, memref<2x2x3x1xf16, #map1>, memref<1xf16, #map2>, memref<7xi64>, memref<1x31x31x1xf16, #map0>) -> ()
// CHECK:           return [[RES_]] : memref<1x31x31x1xf16, #map0>
// CHECK:         }
}

// -----

func @conv_same_padding(%arg0: tensor<1x32x32x3xf32, #zhigh.encoding<{dataLayout = "NHWC"}>>, %arg1: tensor<2x2x3x1xf32, #zhigh.encoding<{dataLayout = "HWCK"}>>, %arg2: tensor<1xf32, #zhigh.encoding<{dataLayout = "1D"}>>) -> tensor<*xf32> {
...
// CHECK-SAME:   ([[PARAM_0_:%.+]]: memref<1x32x32x3xf16, #map0>, [[PARAM_1_:%.+]]: memref<2x2x3x1xf16, #map1>, [[PARAM_2_:%.+]]: memref<1xf16, #map2>) -> memref<1x32x32x1xf16, #map0> {
// CHECK-DAG:       [[VAR_c6_:%.+]] = arith.constant 6 : index
// CHECK-DAG:       [[VAR_c32_i64_:%.+]] = arith.constant 32 : i64
// CHECK-DAG:       [[VAR_c5_:%.+]] = arith.constant 5 : index
// CHECK-DAG:       [[VAR_c4_:%.+]] = arith.constant 4 : index
...
// CHECK-DAG:       [[VAR_c3_i64_:%.+]] = arith.constant 3 : i64
// CHECK-DAG:       [[VAR_c0_:%.+]] = arith.constant 0 : index
// CHECK-DAG:       [[RES_:%.+]] = memref.alloc() {{.*}}: memref<1x32x32x1xf16, #map0>
// CHECK-DAG:       [[RES_1_:%.+]] = memref.alloc() {{.*}}: memref<7xi64>
// CHECK:           krnl.store [[VAR_c1_i64_]], [[RES_1_]]{{.}}[[VAR_c0_]]{{.}} : memref<7xi64>
// CHECK:           krnl.store [[VAR_c3_i64_]], [[RES_1_]]{{.}}[[VAR_c1_]]{{.}} : memref<7xi64>
// CHECK:           krnl.store [[VAR_c32_i64_]], [[RES_1_]]{{.}}[[VAR_c2_]]{{.}} : memref<7xi64>
...
// CHECK:           "zlow.conv2d"([[PARAM_0_]], [[PARAM_1_]], [[PARAM_2_]], [[RES_1_]], [[RES_]]) {act_func = "ACT_NONE", kernel_shape = [2, 2], padding_type = "SAME_PADDING", strides = [1, 1]} : (memref<1x32x32x3xf16, #map0>, memref<2x2x3x1xf16, #map1>, memref<1xf16, #map2>, memref<7xi64>, memref<1x32x32x1xf16, #map0>) -> ()
// CHECK:           return [[RES_]] : memref<1x32x32x1xf16, #map0>
// CHECK:         }
}

// -----

func @conv_valid_padding_unknown_dims(%arg0: tensor<1x?x?x?xf32, #zhigh.encoding<{dataLayout = "NHWC"}>>, %arg1: tensor<2x2x?x1xf32, #zhigh.encoding<{dataLayout = "HWCK"}>>, %arg2: tensor<?xf32, #zhigh.encoding<{dataLayout = "1D"}>>) -> tensor<*xf32> {
...
// CHECK-SAME:   ([[PARAM_0_:%.+]]: memref<1x?x?x?xf16, #map0>, [[PARAM_1_:%.+]]: memref<2x2x?x1xf16, #map1>, [[PARAM_2_:%.+]]: memref<?xf16, #map2>) -> memref<1x?x?x1xf16, #map0> {
// CHECK-DAG:       [[VAR_c6_:%.+]] = arith.constant 6 : index
// CHECK-DAG:       [[VAR_c5_:%.+]] = arith.constant 5 : index
// CHECK-DAG:       [[VAR_c4_:%.+]] = arith.constant 4 : index
// CHECK-DAG:       [[VAR_c1_i64_:%.+]] = arith.constant 1 : i64
...
// CHECK-NOT: separator of consecutive DAGs
// CHECK-DAG:       [[RES_:%.+]] = memref.alloc([[VAR_3_]], [[VAR_4_]]) {{.*}}: memref<1x?x?x1xf16, #map0>
// CHECK-DAG:       [[RES_1_:%.+]] = memref.alloc() {{.*}}: memref<7xi64>
// CHECK:           krnl.store [[VAR_c1_i64_]], [[RES_1_]]{{.}}[[VAR_c0_]]{{.}} : memref<7xi64>
...
// CHECK:           "zlow.conv2d"([[PARAM_0_]], [[PARAM_1_]], [[PARAM_2_]], [[RES_1_]], [[RES_]]) {act_func = "ACT_NONE", kernel_shape = [2, 2], padding_type = "VALID_PADDING", strides = [1, 1]} : (memref<1x?x?x?xf16, #map0>, memref<2x2x?x1xf16, #map1>, memref<?xf16, #map2>, memref<7xi64>, memref<1x?x?x1xf16, #map0>) -> ()
// CHECK:           return [[RES_]] : memref<1x?x?x1xf16, #map0>
// CHECK:         }
}

// -----

func @conv_same_padding_unknown_dims(%arg0: tensor<1x?x?x?xf32, #zhigh.encoding<{dataLayout = "NHWC"}>>, %arg1: tensor<2x2x?x1xf32, #zhigh.encoding<{dataLayout = "HWCK"}>>, %arg2: tensor<?xf32, #zhigh.encoding<{dataLayout = "1D"}>>) -> tensor<*xf32> {
...
// CHECK-DAG:       [[VAR_2_:%.+]] = memref.dim [[PARAM_0_]], [[VAR_c3_]] : memref<1x?x?x?xf16, #map0>
// CHECK-NOT: separator of consecutive DAGs
// CHECK-DAG:       [[RES_:%.+]] = memref.alloc([[VAR_0_]], [[VAR_1_]]) {{.*}}: memref<1x?x?x1xf16, #map0>
// CHECK-DAG:       [[RES_1_:%.+]] = memref.alloc() {{.*}}: memref<7xi64>
// CHECK:           krnl.store [[VAR_c1_i64_]], [[RES_1_]]{{.}}[[VAR_c0_]]{{.}} : memref<7xi64>
...
// CHECK:           "zlow.conv2d"([[PARAM_0_]], [[PARAM_1_]], [[PARAM_2_]], [[RES_1_]], [[RES_]]) {act_func = "ACT_NONE", kernel_shape = [2, 2], padding_type = "SAME_PADDING", strides = [1, 1]} : (memref<1x?x?x?xf16, #map0>, memref<2x2x?x1xf16, #map1>, memref<?xf16, #map2>, memref<7xi64>, memref<1x?x?x1xf16, #map0>) -> ()
// CHECK:           return [[RES_]] : memref<1x?x?x1xf16, #map0>
// CHECK:         }
}
...
func @conv_same_padding_no_bias_unknown_dims(%arg0: tensor<1x32x32x3xf32, #zhigh.encoding<{dataLayout = "NHWC"}>>, %arg1: tensor<2x2x3x1xf32, #zhigh.encoding<{dataLayout = "HWCK"}>>) -> tensor<*xf32> {
...
// CHECK-SAME:   ([[PARAM_0_:%.+]]: memref<1x32x32x3xf16, #map0>, [[PARAM_1_:%.+]]: memref<2x2x3x1xf16, #map1>) -> memref<1x32x32x1xf16, #map0> {
// CHECK-DAG:       [[VAR_c6_:%.+]] = arith.constant 6 : index
// CHECK-DAG:       [[VAR_c32_i64_:%.+]] = arith.constant 32 : i64
// CHECK-DAG:       [[VAR_c5_:%.+]] = arith.constant 5 : index
// CHECK-DAG:       [[VAR_c4_:%.+]] = arith.constant 4 : index
...
// CHECK:           krnl.store [[VAR_c32_i64_]], [[RES_1_]]{{.}}[[VAR_c5_]]{{.}} : memref<7xi64>
// CHECK:           krnl.store [[VAR_c32_i64_]], [[RES_1_]]{{.}}[[VAR_c6_]]{{.}} : memref<7xi64>
// CHECK:           [[VAR_2_:%.+]] = "krnl.global"() {alignment = 4096 : i64, name = "constant_stickify_0", shape = [1, 1, 1, 1, 32, 64], value = opaque<"zhigh", {{.*}}> : tensor<4096xi8>} : () -> memref<1x1x1x1x32x64xf16>
// CHECK:           "zlow.conv2d"([[PARAM_0_]], [[PARAM_1_]], [[VAR_2_]], [[RES_1_]], [[RES_]]) {act_func = "ACT_NONE", kernel_shape = [2, 2], padding_type = "SAME_PADDING", strides = [1, 1]} : (memref<1x32x32x3xf16, #map0>, memref<2x2x3x1xf16, #map1>, memref<1x1x1x1x32x64xf16>, memref<7xi64>, memref<1x32x32x1xf16, #map0>) -> ()
// CHECK:           return [[RES_]] : memref<1x32x32x1xf16, #map0>
// CHECK:         }
}
