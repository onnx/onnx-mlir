// RUN: onnx-mlir-opt --device-placement=load-config-file=%S/load-cfg-all-on-cpu.json --march=z16 --maccel=NNPA --split-input-file %s | FileCheck %s --check-prefix=ALL-ON-CPU

// RUN: onnx-mlir-opt --device-placement=load-config-file=%S/load-cfg-all-relu-on-cpu.json --march=z16 --maccel=NNPA --split-input-file %s | FileCheck %s --check-prefix=ALL-RELU-ON-CPU

// RUN: onnx-mlir-opt --device-placement=load-config-file=%S/load-cfg-not-match-relu.json --march=z16 --maccel=NNPA --split-input-file %s | FileCheck %s --check-prefix=NOT-MATCH-RELU

// RUN: onnx-mlir-opt --device-placement=load-config-file=%S/load-cfg-overlapping-condition.json --march=z16 --maccel=NNPA --split-input-file %s | FileCheck %s --check-prefix=OVERLAPPING
