...
func @test_memory_pool(%arg0: tensor<10x10xf32>) -> tensor<10x10xf32> {
  ...
  // CHECK: [[TMP2:%.+]] = llvm.getelementptr [[TMP1]][%[[MEMPOOL_SIZE]]] : (!llvm.ptr<i8>, !llvm.i64) -> !llvm.ptr<i8>
  // CHECK: [[TYPE_SIZE_IN_BYTES:%.+]] = llvm.ptrtoint [[TMP2]] : !llvm.ptr<i8> to !llvm.i64
  // CHECK: [[ALLOC_MEM_POOL:%.+]] = llvm.call @malloc([[TYPE_SIZE_IN_BYTES]]) : (!llvm.i64) -> !llvm.ptr<i8>
  ...
  // CHECK: [[CONST7:%.+]] = llvm.mlir.constant(10 : index) : !llvm.i64
  // CHECK: [[MUL1:%.+]] = llvm.mul {{.*}}, [[CONST7]] : !llvm.i64
  // CHECK: %[[ADD1:.+]] = llvm.add [[MUL1]], {{.*}} : !llvm.i64
  // CHECK: llvm.getelementptr [[MEM0]][%[[ADD1]]] : (!llvm.ptr<float>, !llvm.i64) -> !llvm.ptr<float>
