...
func @test_pool_bundling(%arg0: memref<10x10xf32>, %arg1: memref<10x20xf32>) -> memref<10x20xf32> {
  ...
  // CHECK: [[MEMPOOL_ALIGNED:%.+]] = memref.alloc() {alignment = 4096 : i64} : memref<2000xi8>
...
func @test_dynamic_pool_bundling(%arg0: memref<?x?xf32>) -> memref<?x10xf32> {
  ...
  %1 = muli %0, %c4 : index
  %2 = muli %1, %c10 : index
  %3 = memref.alloc(%2) {alignment = 4096 : i64} : memref<?xi8>
  ...
  // CHECK: memref.dealloc [[DYN_MEMPOOL_ALIGNED]] : memref<?xi8>
...
func @test_dynamic_and_static_pool_bundling(%arg0: memref<?x?xf32>, %arg1: memref<10x10xf32>) -> memref<?x10xf32> {
  ...
  %const_alloc2 = memref.alloc() {alignment = 4096 : i64} : memref<400xi8>
  ...
  %12 = cmpi "eq", %0, %c1 : index
  %13 = cmpi "eq", %0, %c1 : index
  %15 = memref.alloc(%0) {alignment = 4096 : i64} : memref<?x10xf32>
  ...
  // CHECK: [[STATIC_MEMPOOL_ALIGNED:%.+]] = memref.alloc() {alignment = 4096 : i64} : memref<400xi8>
  ...
  // CHECK: [[RES:%.+]] = memref.alloc([[DIM]]) {alignment = 4096 : i64} : memref<?x10xf32>
  ...
  // CHECK: memref.dealloc [[DYN_MEMPOOL]] : memref<?xi8>
  // CHECK: memref.dealloc [[DYN_MEMPOOL_ALIGNED]] : memref<?xi8>
  // CHECK: memref.dealloc [[STATIC_MEMPOOL_ALIGNED]] : memref<400xi8>
  // CHECK: memref.dealloc [[STATIC_MEMPOOL]] : memref<2400xi8>
  // CHECK: return [[RES]] : memref<?x10xf32>
}
