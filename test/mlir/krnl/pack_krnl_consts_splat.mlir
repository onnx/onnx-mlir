...
// CHECK: [[CONST_PACK:%.+]] = "krnl.packed_const"() {file_name = "test-pack-consts-to-file-splat.bin", is_le = {{[a-z]+}}, size_in_bytes = 0 : i64} : () -> i64
// CHECK-LABEL: func @test_krnl_const_packing_file_splat() -> memref<1x4xf32> {
// CHECK-DAG: [[CONST0:%.+]] = "krnl.global"() {name = "constant_0", offset = 0 : i64, shape = [1, 4], value = dense<0.000000e+00> : tensor<1x4xf32>} : () -> memref<1x4xf32>
// CHECK-DAG: [[CONST1:%.+]] = "krnl.global"() {name = "constant_1", offset = 0 : i64, shape = [1, 4], value = dense<0.000000e+00> : tensor<1x4xf32>} : () -> memref<1x4xf32>
