...
func @test_getref_lowering(%arg0: memref<2x2xf32>) -> memref<2x2xf32> {
  ...
  // CHECK: %[[CONST_100:.+]] = llvm.mlir.constant(100 : index) : !llvm.i64
  // CHECK: [[FLOAT_STAR:%.+]] = llvm.mlir.null : !llvm.ptr<float>
  // CHECK: [[ELEM1:%.+]] = llvm.getelementptr [[FLOAT_STAR]][%[[CONST_100]]] : (!llvm.ptr<float>, !llvm.i64) -> !llvm.ptr<float>
  // CHECK: [[ELEM_SIZE:%.+]] = llvm.ptrtoint [[ELEM1]] : !llvm.ptr<float> to !llvm.i64
  // CHECK: [[MEMPOOL:%.+]] = llvm.call @malloc([[ELEM_SIZE]]) : (!llvm.i64) -> !llvm.ptr<i8>
...
func @test_getref_lowering_dynamic(%arg0: memref<2x2xf32>) -> memref<2x?xf32> {
  ...
  // CHECK: %[[C1_INDEX:.+]] = llvm.mlir.constant(1 : index) : !llvm.i64
  // CHECK: %[[MUL1:.+]] = llvm.mul %[[C5_INDEX]], %[[C10_INDEX]] : !llvm.i64
  // CHECK: [[FLOAT_STAR:%.+]] = llvm.mlir.null : !llvm.ptr<float>
  // CHECK: [[ELEM1:%.+]] = llvm.getelementptr [[FLOAT_STAR]][%[[MUL1]]] : (!llvm.ptr<float>, !llvm.i64) -> !llvm.ptr<float>
  // CHECK: [[ELEM_SIZE:%.+]] = llvm.ptrtoint [[ELEM1]] : !llvm.ptr<float> to !llvm.i64

  /// Allocate the memory pool alloc.
  // CHECK: [[ALLOC:%.+]] = llvm.call @malloc([[ELEM_SIZE]]) : (!llvm.i64) -> !llvm.ptr<i8>
  ...
  // CHECK: [[GETREF_START:%.+]] = llvm.getelementptr [[MEMPOOL]][%[[C13_I64]]] : (!llvm.ptr<float>, !llvm.i64) -> !llvm.ptr<float>
  // CHECK: [[TYPED_GETREF_START:%.+]] = llvm.bitcast [[GETREF_START]] : !llvm.ptr<float> to !llvm.ptr<float>
