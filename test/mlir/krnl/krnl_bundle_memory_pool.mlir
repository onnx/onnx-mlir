...
func @test_pool_bundling(%arg0: memref<10x10xf32>, %arg1: memref<10x20xf32>) -> memref<10x20xf32> {
  ...
  // CHECK: [[CONST_0_INDEX:%.+]] = constant 0 : index
...
func @test_dynamic_pool_bundling(%arg0: memref<?x?xf32>) -> memref<?x10xf32> {
  ...
  // CHECK: [[DYN_MEMPOOL:%.+]] = memref.alloc([[MEMPOOL_SIZE]]) : memref<?xi8>
  ...
  // CHECK: memref.dealloc [[DYN_MEMPOOL]] : memref<?xi8>
...
func @test_dynamic_and_static_pool_bundling(%arg0: memref<?x?xf32>, %arg1: memref<10x10xf32>) -> memref<?x10xf32> {
  ...
  %3 = memref.alloc(%2) : memref<?xi8>
  %4 = "krnl.getref"(%3, %c0_i64, %0) : (memref<?xi8>, i64, index) -> memref<?x10xf32>
  %const_alloc1 = memref.alloc() : memref<800xi8>
  %const_ref1 = "krnl.getref"(%const_alloc1, %c0_i64) : (memref<800xi8>, i64) -> memref<10x20xf32>
  %const_alloc2 = memref.alloc() : memref<400xi8>
  ...
  %13 = cmpi eq, %0, %c1 : index
  %15 = memref.alloc(%0) : memref<?x10xf32>
  %const_alloc3 = memref.alloc() : memref<1600xi8>
  ...
  memref.dealloc %10 : memref<?xi8>
  memref.dealloc %3 : memref<?xi8>
  memref.dealloc %const_alloc1 : memref<800xi8>
  memref.dealloc %const_alloc2 : memref<400xi8>
  memref.dealloc %const_alloc3 : memref<1600xi8>
  return %15 : memref<?x10xf32>
  ...
  // CHECK: [[STATIC_MEMPOOL:%.+]] = memref.alloc() : memref<2800xi8>
  ...
  // CHECK: memref.dealloc [[DYN_MEMPOOL]] : memref<?xi8>
  // CHECK: memref.dealloc [[STATIC_MEMPOOL]] : memref<2800xi8>
  // CHECK: return [[RES]] : memref<?x10xf32>
}
...
func @test_dynamic_pool_rnn(%arg0: memref<1x3x2xf32>, %arg1: memref<1x4x2xf32>, %arg2: memref<1x?x?xf32>) -> memref<1x3x?xf32> attributes {input_names = ["X", "W", "R"], output_names = ["Y"]} {
  ...
  %dim0 = memref.dim %arg1, %c_ind : memref<1x4x2xf32>
  %0 = memref.alloc(%dim0) : memref<1x3x?xf32>
  ...
  krnl.iterate(%2) with (%2 -> %arg3 = 0 to 1) {
    %3:2 = krnl.define_loops 2
    krnl.iterate(%3#0, %3#1) with (%3#0 -> %arg4 = 0 to 3, %3#1 -> %arg5 = 0 to 4) {
      %dim1 = memref.dim %arg2, %c_ind : memref<1x?x?xf32>
      %4 = memref.alloc(%dim1) : memref<?xi8>
      %5 = "krnl.getref"(%4, %c0_i64) : (memref<?xi8>, i64) -> memref<f32>
      %6 = krnl.load %0[%c0, %arg4, %arg5] : memref<1x3x?xf32>
      %7 = memref.alloc(%dim1) : memref<?xi8>
      %8 = "krnl.getref"(%7, %c0_i64) : (memref<?xi8>, i64) -> memref<f32>
      krnl.store %cst, %8[] : memref<f32>
      %c_ind_2 = constant 2 : index
      %dim2 = memref.dim %arg2, %c_ind_2 : memref<1x?x?xf32>
      %9 = memref.alloc(%dim2) : memref<?xi8>
      ...
      %13 = krnl.load %10[] : memref<f32>
      %14 = addf %12, %13 : f32
      %15 = memref.alloc(%dim2) : memref<?xi8>
  ...
  // CHECK: krnl.define_loops 2
  // CHECK: krnl.iterate
  // CHECK: [[DIM1:%.+]] = memref.dim %arg2, [[C1]] : memref<1x?x?xf32>
  // CHECK: [[DIM2:%.+]] = memref.dim %arg2, [[C2]] : memref<1x?x?xf32>
  ...
  // CHECK: [[DYNAMIC_MEMORY_POOL:%.+]] = memref.alloc([[ADD3]]) : memref<?xi8>
  ...
  // CHECK: memref.dealloc [[DYNAMIC_MEMORY_POOL]] : memref<?xi8>
}
