...
// CHECK-DAG:   [[MAP_0_:#.+]] = affine_map<(d0, d1) -> ({{-d0 \+ d1|d0 - d1}})>
// CHECK-DAG:   [[MAP_1_:#.+]] = affine_map<(d0, d1, d2, d3) -> ({{-d0 \+ d1 - d2 \+ d3|d0 - d1 \+ d2 - d3}})>
...
// CHECK:               [[VAR_5_:%.+]] = affine.apply [[MAP_0_]]({{.*}})
...
// CHECK-DAG:           [[VAR_9_:%.+]] = affine.apply [[MAP_0_]]({{.*}})
// CHECK-NOT: separator of consecutive DAGs
