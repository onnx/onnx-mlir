...
//CHECK-DAG: %[[CONST1:.*]] = torch.constant.int 0
//CHECK-DAG: [[STRIDE:%.]] = torch.prim.ListConstruct %{{[^,]*}}, %{{[^,]*}} : (!torch.int, !torch.int) -> !torch.list<int>
//CHECK-DAG: [[PAD:%.]] = torch.prim.ListConstruct %[[CONST1]], %[[CONST1]] : (!torch.int, !torch.int) -> !torch.list<int>
...
//CHECK: torch.aten.add.Tensor [[RES]], %{{[^,]*}}, %{{[^,]*}} : !torch.vtensor<[2,4,5,3],f32>, !torch.vtensor<[],f32>, !torch.int -> !torch.vtensor<[2,4,5,3],f32>
