...
// CHECK-DAG: #map1 = affine_map<()[s0] -> (-s0 + 39, 4)>
// CHECK-DAG: #map2 = affine_map<()[s0] -> (-s0 + 56, 6)>
// CHECK-LABEL:  func private @copy_to_runtime_start_indices
// CHECK-SAME:   ([[START0_:%.+]]: index, [[START1_:%.+]]: index) attributes {llvm.emit_c_interface} {
// CHECK-DAG:       [[ORGINAL_:%.+]] = memref.alloca() : memref<39x56xf32>
// CHECK-DAG:       [[BUFFER_:%.+]] = memref.alloca() : memref<4x6xf32>
// CHECK:           affine.for [[I_0_:%.+]] = 0 to min #map1(){{.}}[[START0_]]{{.}} {
// CHECK:             affine.for [[I_1_:%.+]] = 0 to min #map2(){{.}}[[START1_]]{{.}} {
...
// CHECK:               affine.store [[LOAD_ORGINAL_MEM_]], [[BUFFER_]]{{.}}[[I_0_]], [[I_1_]]{{.}} : memref<4x6xf32>
// CHECK:             }
// CHECK:             affine.for [[I_2_:%.+]] = max #map2(){{.}}[[START1_]]{{.}} to [[VAR_7_]] {
// CHECK:               affine.store [[ZERO_]], [[BUFFER_]]{{.}}[[I_0_]], [[I_2_]]{{.}} : memref<4x6xf32>
// CHECK:             }
// CHECK:           }
// CHECK:           affine.for [[I_3_:%.+]] = max #map1(){{.}}[[START0_]]{{.}} to [[VAR_4_]] {
...
// CHECK-DAG: #map2 = affine_map<()[s0] -> (-s0 + 39, 4)>
// CHECK-DAG: #map3 = affine_map<()[s0] -> (-s0 + 56, 6)>
...
// CHECK-DAG:       [[BUFFER_:%.+]] = memref.alloca() : memref<4x6xf32>
// CHECK-DAG:       [[VAR_2_:%.+]] = affine.min #map2(){{.}}[[START0_]]{{.}}
// CHECK:           [[VAR_3_:%.+]] = arith.ceildivsi [[VAR_2_]], [[CST_3_]] : index
// CHECK-DAG:       [[VAR_4_:%.+]] = arith.muli [[VAR_3_]], [[CST_3_]] : index
// CHECK-DAG:       [[VAR_5_:%.+]] = affine.min #map3(){{.}}[[START1_]]{{.}}
// CHECK:           [[VAR_6_:%.+]] = arith.ceildivsi [[VAR_5_]], [[CST_3_]] : index
// CHECK:           [[VAR_7_:%.+]] = arith.muli [[VAR_6_]], [[CST_3_]] : index
// CHECK:           affine.for [[I_0_:%.+]] = 0 to min #map2(){{.}}[[START0_]]{{.}} {
// CHECK:             affine.for [[I_1_:%.+]] = 0 to min #map3(){{.}}[[START1_]]{{.}} {
// CHECK:               [[LOAD_ORGINAL_MEM_:%.+]] = affine.load [[ORGINAL_]][2, 5, [[I_0_]] + symbol([[START0_]]), [[I_1_]] + symbol([[START1_]])] : memref<5x10x39x56xf32>
// CHECK:               affine.store [[LOAD_ORGINAL_MEM_]], [[BUFFER_]]{{.}}[[I_0_]], [[I_1_]]{{.}} : memref<4x6xf32>
// CHECK:             }
// CHECK:             affine.for [[I_2_:%.+]] = max #map3(){{.}}[[START1_]]{{.}} to [[VAR_7_]] {
// CHECK:               affine.store [[ZERO_]], [[BUFFER_]]{{.}}[[I_0_]], [[I_2_]]{{.}} : memref<4x6xf32>
// CHECK:             }
// CHECK:           }
// CHECK:           affine.for [[I_3_:%.+]] = max #map2(){{.}}[[START0_]]{{.}} to [[VAR_4_]] {
// CHECK:             affine.for [[I_4_:%.+]] = 0 to [[VAR_7_]] {
// CHECK:               affine.store [[ZERO_]], [[BUFFER_]]{{.}}[[I_3_]], [[I_4_]]{{.}} : memref<4x6xf32>
// CHECK:             }
// CHECK:           }
...
// CHECK-DAG: #map = affine_map<(d0, d1) -> (d0, d1)>
// CHECK-DAG: #map1 = affine_map<(d0) -> (-d0 + 45, 10)>
...
// CHECK:             affine.for [[I_1_:%.+]] = 0 to min #map1([[I_0_]]) {
// CHECK:               affine.for [[I_2_:%.+]] = 0 to 60 {
