...
func private @matmulKrnl_full_tiles(%A: memref<4x6xf32>, %B: memref<6x8xf32>, %C: memref<4x8xf32>) {
    ...
    krnl.iterate(%ib, %jb, %kb) with (%ii -> %i = 0 to 4, %jj -> %j = 0 to 8, %kk -> %k = 0 to 6) {
        ...
        krnl.matmul %A, %B, %C, [%c0, %c0, %c0], [%c4, %c8, %c6], (%il, %jl, %kl) {unroll=false, simdize=true} : 
            memref<4x6xf32>, memref<6x8xf32>, memref<4x8xf32>, (!krnl.loop, !krnl.loop, !krnl.loop)
    }
    return
// mlir2FileCheck.py -a'["A", "B", "C"]' -n'{"0": "B_VEC", "1": "C_VEC"}'
...
// CHECK-DAG:                 [[LOAD_C_VEC_MEM_:%.+]] = affine.load [[C_VEC_]]{{.}}[[I_3_]], 0] : memref<4x1xvector<8xf32>>
...
// CHECK-DAG:                   [[LOAD_B_VEC_MEM_:%.+]] = affine.load [[B_VEC_]]{{.}}[[I_4_]], 0] : memref<6x1xvector<8xf32>>
...
// CHECK:                     affine.store [[LOAD_RES_MEM_1_]], [[C_VEC_]]{{.}}[[I_3_]], 0] : memref<4x1xvector<8xf32>>
// CHECK:                   }
// CHECK:                 }
// CHECK:               }
// CHECK:             }
...
func @matmulKrnl_runtime(%A: memref<4x6xf32>, %B: memref<6x8xf32>, %C: memref<4x8xf32>, 
    ...
    krnl.permute(%ib, %il, %jb, %jl, %kb, %kl) [0, 3, 1, 4, 2, 5] : !krnl.loop, !krnl.loop, !krnl.loop, !krnl.loop, !krnl.loop, !krnl.loop
    krnl.iterate(%ib, %jb, %kb) with (%ii -> %i = 0 to 4, %jj -> %j = 0 to 8, %kk -> %k = 0 to 6) {
        krnl.matmul %A, %B, %C, [%sn, %sm, %sk], [%dn, %dm, %dk], (%il, %jl, %kl) {simdize=true, unroll=false} : 
          memref<4x6xf32>, memref<6x8xf32>, memref<4x8xf32>, (!krnl.loop, !krnl.loop, !krnl.loop)
    }
    return
...
// CHECK:                   affine.for [[I_3_:%.+]] = 0 to 4 {
// CHECK-DAG:                 [[RES_:%.+]] = alloca() : memref<vector<8xf32>>
// CHECK-DAG:                 [[LOAD_VAR_1_MEM_:%.+]] = affine.load [[VAR_1_]]{{.}}[[I_3_]] + symbol([[PARAM_0_]]) - (symbol([[PARAM_0_]]) floordiv 4) * 4, (symbol([[PARAM_1_]]) mod 8) ceildiv 8] : memref<4x1xvector<8xf32>>
// CHECK:                     affine.store [[LOAD_VAR_1_MEM_]], [[RES_]][] : memref<vector<8xf32>>
// CHECK:                     affine.for [[I_4_:%.+]] = 0 to 6 {
// CHECK:                       [[LOAD_A_MEM_:%.+]] = affine.load [[A_]]{{.}}[[I_3_]] + symbol([[PARAM_0_]]) - (symbol([[PARAM_0_]]) floordiv 4) * 4, [[I_4_]] + symbol([[PARAM_2_]]) - (symbol([[PARAM_2_]]) floordiv 6) * 6] : memref<4x6xf32>
// CHECK-DAG:                   [[VAR_6_:%.+]] = vector.broadcast [[LOAD_A_MEM_]] : f32 to vector<8xf32>
// CHECK-DAG:                   [[LOAD_VAR_0_MEM_:%.+]] = affine.load [[VAR_0_]]{{.}}[[I_4_]] + symbol([[PARAM_2_]]) - (symbol([[PARAM_2_]]) floordiv 6) * 6, (symbol([[PARAM_1_]]) mod 8) ceildiv 8] : memref<6x1xvector<8xf32>>
// CHECK-DAG:                   [[LOAD_RES_MEM_:%.+]] = affine.load [[RES_]][] : memref<vector<8xf32>>
// CHECK:                       [[VAR_9_:%.+]] = vector.fma [[VAR_6_]], [[LOAD_VAR_0_MEM_]], [[LOAD_RES_MEM_]] : vector<8xf32>
// CHECK:                       affine.store [[VAR_9_]], [[RES_]][] : memref<vector<8xf32>>
// CHECK:                     }
// CHECK:                     [[LOAD_RES_MEM_1_:%.+]] = affine.load [[RES_]][] : memref<vector<8xf32>>
// CHECK:                     affine.store [[LOAD_RES_MEM_1_]], [[VAR_1_]]{{.}}[[I_3_]] + symbol([[PARAM_0_]]) - (symbol([[PARAM_0_]]) floordiv 4) * 4, (symbol([[PARAM_1_]]) mod 8) ceildiv 8] : memref<4x1xvector<8xf32>>
// CHECK:                   }
// CHECK:                 } else {
...
// CHECK:                     affine.for [[I_5_:%.+]] = 0 to min #map0(){{.}}[[PARAM_0_]], [[PARAM_3_]]{{.}} {
// CHECK-DAG:                   [[RES_1_:%.+]] = alloca() : memref<vector<8xf32>>
// CHECK-DAG:                   [[LOAD_VAR_1_MEM_1_:%.+]] = affine.load [[VAR_1_1_]]{{.}}[[I_5_]] + symbol([[PARAM_0_]]) - (symbol([[PARAM_0_]]) floordiv 4) * 4, (symbol([[PARAM_1_]]) mod 8) ceildiv 8] : memref<4x1xvector<8xf32>>
// CHECK:                       affine.store [[LOAD_VAR_1_MEM_1_]], [[RES_1_]][] : memref<vector<8xf32>>
// CHECK:                       affine.for [[I_6_:%.+]] = 0 to min #map1(){{.}}[[PARAM_2_]], [[PARAM_5_]]{{.}} {
// CHECK:                         [[LOAD_A_MEM_1_:%.+]] = affine.load [[A_]]{{.}}[[I_5_]] + symbol([[PARAM_0_]]) - (symbol([[PARAM_0_]]) floordiv 4) * 4, [[I_6_]] + symbol([[PARAM_2_]]) - (symbol([[PARAM_2_]]) floordiv 6) * 6] : memref<4x6xf32>
// CHECK-DAG:                     [[VAR_6_1_:%.+]] = vector.broadcast [[LOAD_A_MEM_1_]] : f32 to vector<8xf32>
// CHECK-DAG:                     [[LOAD_VAR_0_MEM_1_:%.+]] = affine.load [[VAR_0_1_]]{{.}}[[I_6_]] + symbol([[PARAM_2_]]) - (symbol([[PARAM_2_]]) floordiv 6) * 6, (symbol([[PARAM_1_]]) mod 8) ceildiv 8] : memref<6x1xvector<8xf32>>
...
// CHECK:                       affine.store [[LOAD_RES_MEM_1_]], [[VAR_1_1_]]{{.}}[[I_5_]] + symbol([[PARAM_0_]]) - (symbol([[PARAM_0_]]) floordiv 4) * 4, (symbol([[PARAM_1_]]) mod 8) ceildiv 8] : memref<4x1xvector<8xf32>>
// CHECK:                     }
// CHECK:                   } else {
// CHECK:                     affine.for [[I_7_:%.+]] = 0 to min #map0(){{.}}[[PARAM_0_]], [[PARAM_3_]]{{.}} {
// CHECK:                       affine.for [[I_8_:%.+]] = 0 to #map2(){{.}}[[PARAM_1_]], [[PARAM_4_]]{{.}} {
// CHECK-DAG:                     [[RES_2_:%.+]] = alloca() : memref<f32>
// CHECK-DAG:                     [[VAR_1_1_:%.+]] = affine.load [[C_]]{{.}}[[I_7_]] + symbol([[PARAM_0_]]) - (symbol([[PARAM_0_]]) floordiv 4) * 4, [[I_8_]] + symbol([[PARAM_1_]]) - (symbol([[PARAM_1_]]) floordiv 8) * 8] : memref<4x8xf32>
// CHECK:                         affine.store [[VAR_1_1_]], [[RES_2_]][] : memref<f32>
// CHECK:                         affine.for [[I_9_:%.+]] = 0 to min #map1(){{.}}[[PARAM_2_]], [[PARAM_5_]]{{.}} {
// CHECK-DAG:                       [[LOAD_VAR_1_MEM_1_:%.+]] = affine.load [[B_]]{{.}}[[B_]]4 + symbol([[PARAM_2_]]) - (symbol([[PARAM_2_]]) floordiv 6) * 6, [[B_]]3 + symbol([[PARAM_1_]]) - (symbol([[PARAM_1_]]) floordiv 8) * 8] : memref<6x8xf32>
// CHECK-DAG:                       [[LOAD_A_MEM_2_:%.+]] = affine.load [[A_]]{{.}}[[I_7_]] + symbol([[PARAM_0_]]) - (symbol([[PARAM_0_]]) floordiv 4) * 4, [[I_9_]] + symbol([[PARAM_2_]]) - (symbol([[PARAM_2_]]) floordiv 6) * 6] : memref<4x6xf32>
...
// CHECK:                         affine.store [[RES_1_]], [[C_]]{{.}}[[I_7_]] + symbol([[PARAM_0_]]) - (symbol([[PARAM_0_]]) floordiv 4) * 4, [[I_8_]] + symbol([[PARAM_1_]]) - (symbol([[PARAM_1_]]) floordiv 8) * 8] : memref<4x8xf32>
// CHECK:                       }
// CHECK:                     }
// CHECK:                   }
// CHECK:                 }
