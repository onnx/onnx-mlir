...
// PARSE_CONFIG_FILE_1-DAG: Onnx-mlir command: {{.*}}onnx-mlir -v --config-file {{.*}}custom-omconfig.json {{.*}}.mlir -o {{.*}}test4 -O3 -march=z16
// PARSE_CONFIG_FILE_1-DAG: {{.*}}opt -O3{{.*}}
// PARSE_CONFIG_FILE_1-DAG: {{.*}}llc -O3{{.*}}

// COM: Check parsing the compile options "-config-file config.json" 
// PARSE_CONFIG_FILE_2-DAG: Config file: {{.*}}custom-omconfig.json 
// PARSE_CONFIG_FILE_2-DAG: Onnx-mlir command: {{.*}}onnx-mlir -v -config-file {{.*}}custom-omconfig.json {{.*}}.mlir -o {{.*}}test5 -O3 -march=z16
