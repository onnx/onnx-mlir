...
// COM: func @test_conv_batchnormtestmode_fusion_nobias(%arg0 : tensor<1x3x224x224xf32>) -> tensor<1x64x112x112xf32> {
// COM:     %cst = constant unit
// COM:     %0 = "onnx.Constant"() : () -> tensor<64x3x7x7xf32>
// COM:     %1 = "onnx.Conv"(%arg0, %0, %cst) {auto_pad = "NOTSET", dilations = [1, 1], group = 1 : si64, kernel_shape = [7, 7], pads = [3, 3, 3, 3], strides = [2, 2]} : (tensor<1x3x224x224xf32>, tensor<64x3x7x7xf32>, none) -> tensor<1x64x112x112xf32>
// COM:     %2 = "onnx.Constant"() : () -> tensor<64xf32>
// COM:     %3 = "onnx.Constant"() : () -> tensor<64xf32>
// COM:     %4 = "onnx.Constant"() : () -> tensor<64xf32>
// COM:     %5 = "onnx.Constant"() : () -> tensor<64xf32>
// COM:     %6 = "onnx.BatchNormalizationTestMode"(%1, %2, %3, %4, %5) {epsilon = 1.00000007E-5 : f32} : (tensor<1x64x112x112xf32>, tensor<64xf32>, tensor<64xf32>, tensor<64xf32>, tensor<64xf32>) -> tensor<1x64x112x112xf32>
// COM:     return %6 :  tensor<1x64x112x112xf32>
// COM: 
// COM:     // CHECK-LABEL: test_conv_batchnormtestmode_fusion_nobias
// COM:     // CHECK: [[WEIGHT:%.+]] = "onnx.Constant"() : () -> tensor<64x3x7x7xf32>
// COM:     // CHECK: [[SCALE:%.+]] = "onnx.Constant"() : () -> tensor<64xf32>
// COM:     // CHECK: [[B:%.+]] = "onnx.Constant"() : () -> tensor<64xf32>
// COM:     // CHECK: [[MEAN:%.+]] = "onnx.Constant"() : () -> tensor<64xf32>
// COM:     // CHECK: [[VARIANCE:%.+]] = "onnx.Constant"() : () -> tensor<64xf32>
// COM:     // CHECK: [[EPSILON:%.+]] = "onnx.Constant"() {value = dense<1.00000007E-5> : tensor<1xf32>} : () -> tensor<1xf32>
// COM: 
// COM:     // CHECK: [[VAR_EPSILON:%.+]] = "onnx.Add"([[VARIANCE]], [[EPSILON]]) : (tensor<64xf32>, tensor<1xf32>) -> tensor<64xf32>
// COM:     // CHECK: [[SQRT:%.+]] = "onnx.Sqrt"([[VAR_EPSILON]]) : (tensor<64xf32>) -> tensor<*xf32>
// COM:     // CHECK: [[COEFFICIENT_W:%.+]] = "onnx.Div"([[SCALE]], [[SQRT]]) : (tensor<64xf32>, tensor<*xf32>) -> tensor<*xf32>
// COM:     // CHECK: [[UNSQUEEZE:%.+]] = "onnx.Unsqueeze"([[COEFFICIENT_W]]) {axes = [1, 2, 3]} : (tensor<*xf32>) -> tensor<*xf32>
// COM:     // CHECK: [[NEW_WEIGHT:%.+]] = "onnx.Mul"([[WEIGHT]], [[UNSQUEEZE]]) : (tensor<64x3x7x7xf32>, tensor<*xf32>) -> tensor<*xf32>
// COM: 
// COM:     // CHECK: [[NEG_MEAN:%.+]] = "onnx.Neg"([[MEAN]]) : (tensor<64xf32>) -> tensor<*xf32>
// COM:     // CHECK: [[MUL:%.+]] = "onnx.Mul"([[COEFFICIENT_W]], [[NEG_MEAN]]) : (tensor<*xf32>, tensor<*xf32>) -> tensor<*xf32>
// COM:     // CHECK: [[NEW_BIAS:%.+]] = "onnx.Add"([[B]], [[MUL]]) : (tensor<64xf32>, tensor<*xf32>) -> tensor<*xf32>
// COM: 
// COM:     // CHECK: [[PAD_ARG1:%.+]] = "onnx.Constant"() {value = dense<[0, 0, 3, 3, 0, 0, 3, 3]> : tensor<8xi64>} : () -> tensor<8xi64>
// COM:     // CHECK: [[PAD_ARG2:%.+]] = "onnx.Constant"() {value = dense<0.000000e+00> : tensor<1xf32>} : () -> tensor<1xf32>
// COM:     // CHECK: [[PADDED_INPUT:%.+]] = "onnx.Pad"(%arg0, [[PAD_ARG1]], [[PAD_ARG2]]) {mode = "constant"} : (tensor<1x3x224x224xf32>, tensor<8xi64>, tensor<1xf32>) -> tensor<*xf32>
// COM: 
// COM:     // CHECK: [[RES:%.+]] = "onnx.Conv"([[PADDED_INPUT]], [[NEW_WEIGHT]], [[NEW_BIAS]]) {auto_pad = "NOTSET", dilations = [1, 1], group = 1 : si64, kernel_shape = [7, 7], pads = [0, 0, 0, 0], strides = [2, 2]} : (tensor<*xf32>, tensor<*xf32>, tensor<*xf32>) -> tensor<1x64x112x112xf32> 
// COM: 
// COM:     // CHECK-NOT: {{.*}} = "onnx.BatchNormalizationTestMode"{{.*}}
// COM: 
// COM:     // CHECK: return [[RES]] : tensor<1x64x112x112xf32>
// COM: }

// -----

// COM: func @test_conv_batchnormtestmode_fusion(%arg0 : tensor<1x3x224x224xf32>, %arg1 : tensor<64xf32>) -> tensor<1x64x112x112xf32> {
// COM:     %cst = constant unit
// COM:     %0 = "onnx.Constant"() : () -> tensor<64x3x7x7xf32>
// COM:     %1 = "onnx.Conv"(%arg0, %0, %arg1) {auto_pad = "NOTSET", dilations = [1, 1], group = 1 : si64, kernel_shape = [7, 7], pads = [3, 3, 3, 3], strides = [2, 2]} : (tensor<1x3x224x224xf32>, tensor<64x3x7x7xf32>, tensor<64xf32>) -> tensor<1x64x112x112xf32>
// COM:     %2 = "onnx.Constant"() : () -> tensor<64xf32>
// COM:     %3 = "onnx.Constant"() : () -> tensor<64xf32>
// COM:     %4 = "onnx.Constant"() : () -> tensor<64xf32>
// COM:     %5 = "onnx.Constant"() : () -> tensor<64xf32>
...
// COM:     return %6 :  tensor<1x64x112x112xf32>
// COM: 
// COM:     // CHECK-LABEL: test_conv_batchnormtestmode_fusion
// COM:     // CHECK: [[WEIGHT:%.+]] = "onnx.Constant"() : () -> tensor<64x3x7x7xf32>
// COM:     // CHECK: [[SCALE:%.+]] = "onnx.Constant"() : () -> tensor<64xf32>
// COM:     // CHECK: [[B:%.+]] = "onnx.Constant"() : () -> tensor<64xf32>
// COM:     // CHECK: [[MEAN:%.+]] = "onnx.Constant"() : () -> tensor<64xf32>
...
// COM:     // CHECK: [[NEW_WEIGHT:%.+]] = "onnx.Mul"([[WEIGHT]], [[UNSQUEEZE]]) : (tensor<64x3x7x7xf32>, tensor<*xf32>) -> tensor<*xf32>
// COM: 
// COM:     // CHECK: [[SUB:%.+]] = "onnx.Sub"(%arg1, [[MEAN]]) : (tensor<64xf32>, tensor<64xf32>) -> tensor<64xf32>
// COM:     // CHECK: [[MUL:%.+]] = "onnx.Mul"([[COEFFICIENT_W]], [[SUB]]) : (tensor<*xf32>, tensor<64xf32>) -> tensor<*xf32>
// COM:     // CHECK: [[NEW_BIAS:%.+]] = "onnx.Add"([[B]], [[MUL]]) : (tensor<64xf32>, tensor<*xf32>) -> tensor<*xf32>
// COM: 
// COM:     // CHECK: [[PAD_ARG1:%.+]] = "onnx.Constant"() {value = dense<[0, 0, 3, 3, 0, 0, 3, 3]> : tensor<8xi64>} : () -> tensor<8xi64>
// COM:     // CHECK: [[PAD_ARG2:%.+]] = "onnx.Constant"() {value = dense<0.000000e+00> : tensor<1xf32>} : () -> tensor<1xf32>
