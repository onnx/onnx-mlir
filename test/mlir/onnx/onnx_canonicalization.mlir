...
func.func @test_div_in_attention(%arg0: tensor<?x?x768xf32>, %arg1: tensor<?x?x768xf32>) -> tensor<?x12x?x?xf32> {
  ...
  %4 = "onnx.MatMul"(%arg1, %3) {onnx_node_name = "/encoder/layer.0/attention/self/query/MatMul"} : (tensor<?x?x768xf32>, tensor<768x768xf32>) -> tensor<?x?x768xf32>
  %5 = "onnx.Add"(%4, %0) {onnx_node_name = "/encoder/layer.0/attention/self/query/Add-Initializer_encoder.layer.0.attention.self.query.bias_193"} : (tensor<?x?x768xf32>, tensor<768xf32>) -> tensor<?x?x768xf32>
  %6 = "onnx.Dim"(%5) {axis = 0 : si64, onnx_node_name = "/encoder/layer.0/attention/self/Shape_4_51"} : (tensor<?x?x768xf32>) -> tensor<1xi64>
  %7 = "onnx.Dim"(%5) {axis = 1 : si64, onnx_node_name = "/encoder/layer.0/attention/self/Shape_5_205"} : (tensor<?x?x768xf32>) -> tensor<1xi64>
  %8 = "onnx.Concat"(%6, %7, %2, %1) {axis = 0 : si64, onnx_node_name = "/encoder/layer.0/attention/self/Concat_2"} : (tensor<1xi64>, tensor<1xi64>, tensor<1xi64>, tensor<1xi64>) -> tensor<4xi64>
  %9 = "onnx.Reshape"(%5, %8) {allowzero = 0 : si64, onnx_node_name = "/encoder/layer.0/attention/self/Reshape_2"} : (tensor<?x?x768xf32>, tensor<4xi64>) -> tensor<?x?x12x64xf32>
  %10 = "onnx.Transpose"(%9) {onnx_node_name = "/encoder/layer.0/attention/self/Transpose_1", perm = [0, 2, 1, 3]} : (tensor<?x?x12x64xf32>) -> tensor<?x12x?x64xf32>
  %11 = "onnx.MatMul"(%arg0, %3) {onnx_node_name = "/encoder/layer.0/attention/self/key/MatMul"} : (tensor<?x?x768xf32>, tensor<768x768xf32>) -> tensor<?x?x768xf32>
  %12 = "onnx.Add"(%11, %0) {onnx_node_name = "/encoder/layer.0/attention/self/key/Add-Initializer_encoder.layer.0.attention.self.key.bias_127"} : (tensor<?x?x768xf32>, tensor<768xf32>) -> tensor<?x?x768xf32>
  %13 = "onnx.Dim"(%12) {axis = 0 : si64, onnx_node_name = "/encoder/layer.0/attention/self/Shape_124"} : (tensor<?x?x768xf32>) -> tensor<1xi64>
  %14 = "onnx.Dim"(%12) {axis = 1 : si64, onnx_node_name = "/encoder/layer.0/attention/self/Shape_1_133"} : (tensor<?x?x768xf32>) -> tensor<1xi64>
  %15 = "onnx.Concat"(%13, %14, %2, %1) {axis = 0 : si64, onnx_node_name = "/encoder/layer.0/attention/self/Concat"} : (tensor<1xi64>, tensor<1xi64>, tensor<1xi64>, tensor<1xi64>) -> tensor<4xi64>
  %16 = "onnx.Reshape"(%12, %15) {allowzero = 0 : si64, onnx_node_name = "/encoder/layer.0/attention/self/Reshape"} : (tensor<?x?x768xf32>, tensor<4xi64>) -> tensor<?x?x12x64xf32>
  %17 = "onnx.Transpose"(%16) {onnx_node_name = "/encoder/layer.0/attention/self/Transpose_2", perm = [0, 2, 3, 1]} : (tensor<?x?x12x64xf32>) -> tensor<?x12x64x?xf32>
  %18 = "onnx.MatMul"(%10, %17) {onnx_node_name = "/encoder/layer.0/attention/self/MatMul"} : (tensor<?x12x?x64xf32>, tensor<?x12x64x?xf32>) -> tensor<?x12x?x?xf32>
  %19 = onnx.Constant dense<8.000000e+00> : tensor<f32>
  %20 = "onnx.Div"(%18, %19) {onnx_node_name = "/encoder/layer.0/attention/self/Div"} : (tensor<?x12x?x?xf32>, tensor<f32>) -> tensor<?x12x?x?xf32>
...
// CHECK-DAG:       [[VAR_6_:%.+]] = "onnx.MatMul"([[PARAM_1_]], [[VAR_5_]]) {onnx_node_name = "/encoder/layer.0/attention/self/query/MatMul"} : (tensor<?x?x768xf32>, tensor<768x768xf32>) -> tensor<?x?x768xf32>
// CHECK-DAG:       [[VAR_7_:%.+]] = "onnx.Div"([[VAR_0_]], [[VAR_4_]]) : (tensor<768xf32>, tensor<f32>) -> tensor<768xf32>
// CHECK:           [[VAR_8_:%.+]] = "onnx.Add"([[VAR_6_]], [[VAR_7_]]) {onnx_node_name = "/encoder/layer.0/attention/self/query/Add-Initializer_encoder.layer.0.attention.self.query.bias_193"} : (tensor<?x?x768xf32>, tensor<768xf32>) -> tensor<?x?x768xf32>
// CHECK-DAG:       [[VAR_9_:%.+]] = "onnx.Dim"([[VAR_8_]]) {axis = 0 : si64, onnx_node_name = "/encoder/layer.0/attention/self/Shape_4_51"} : (tensor<?x?x768xf32>) -> tensor<1xi64>
// CHECK-DAG:       [[VAR_10_:%.+]] = "onnx.Dim"([[VAR_8_]]) {axis = 1 : si64, onnx_node_name = "/encoder/layer.0/attention/self/Shape_5_205"} : (tensor<?x?x768xf32>) -> tensor<1xi64>
// CHECK:           [[VAR_11_:%.+]] = "onnx.Concat"([[VAR_9_]], [[VAR_10_]], [[VAR_2_]], [[VAR_1_]]) {axis = 0 : si64, onnx_node_name = "/encoder/layer.0/attention/self/Concat_2"} : (tensor<1xi64>, tensor<1xi64>, tensor<1xi64>, tensor<1xi64>) -> tensor<4xi64>
// CHECK:           [[VAR_12_:%.+]] = "onnx.Reshape"([[VAR_8_]], [[VAR_11_]]) {allowzero = 0 : si64, onnx_node_name = "/encoder/layer.0/attention/self/Reshape_2"} : (tensor<?x?x768xf32>, tensor<4xi64>) -> tensor<?x?x12x64xf32>
// CHECK-DAG:       [[VAR_13_:%.+]] = "onnx.Transpose"([[VAR_12_]]) {onnx_node_name = "/encoder/layer.0/attention/self/Transpose_1", perm = [0, 2, 1, 3]} : (tensor<?x?x12x64xf32>) -> tensor<?x12x?x64xf32>
// CHECK-DAG:       [[VAR_14_:%.+]] = "onnx.MatMul"([[PARAM_0_]], [[VAR_3_]]) {onnx_node_name = "/encoder/layer.0/attention/self/key/MatMul"} : (tensor<?x?x768xf32>, tensor<768x768xf32>) -> tensor<?x?x768xf32>
// CHECK:           [[VAR_15_:%.+]] = "onnx.Add"([[VAR_14_]], [[VAR_0_]]) {onnx_node_name = "/encoder/layer.0/attention/self/key/Add-Initializer_encoder.layer.0.attention.self.key.bias_127"} : (tensor<?x?x768xf32>, tensor<768xf32>) -> tensor<?x?x768xf32>
// CHECK-DAG:       [[VAR_16_:%.+]] = "onnx.Dim"([[VAR_15_]]) {axis = 0 : si64, onnx_node_name = "/encoder/layer.0/attention/self/Shape_124"} : (tensor<?x?x768xf32>) -> tensor<1xi64>
// CHECK-DAG:       [[VAR_17_:%.+]] = "onnx.Dim"([[VAR_15_]]) {axis = 1 : si64, onnx_node_name = "/encoder/layer.0/attention/self/Shape_1_133"} : (tensor<?x?x768xf32>) -> tensor<1xi64>
// CHECK:           [[VAR_18_:%.+]] = "onnx.Concat"([[VAR_16_]], [[VAR_17_]], [[VAR_2_]], [[VAR_1_]]) {axis = 0 : si64, onnx_node_name = "/encoder/layer.0/attention/self/Concat"} : (tensor<1xi64>, tensor<1xi64>, tensor<1xi64>, tensor<1xi64>) -> tensor<4xi64>
// CHECK:           [[VAR_19_:%.+]] = "onnx.Reshape"([[VAR_15_]], [[VAR_18_]]) {allowzero = 0 : si64, onnx_node_name = "/encoder/layer.0/attention/self/Reshape"} : (tensor<?x?x768xf32>, tensor<4xi64>) -> tensor<?x?x12x64xf32>
// CHECK:           [[VAR_20_:%.+]] = "onnx.Transpose"([[VAR_19_]]) {onnx_node_name = "/encoder/layer.0/attention/self/Transpose_2", perm = [0, 2, 3, 1]} : (tensor<?x?x12x64xf32>) -> tensor<?x12x64x?xf32>
// CHECK:           [[VAR_21_:%.+]] = "onnx.MatMul"([[VAR_13_]], [[VAR_20_]]) {onnx_node_name = "/encoder/layer.0/attention/self/MatMul"} : (tensor<?x12x?x64xf32>, tensor<?x12x64x?xf32>) -> tensor<?x12x?x?xf32>
