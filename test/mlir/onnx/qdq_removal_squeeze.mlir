// RUN: onnx-mlir-opt --canonicalize --qdq-around-op-opt-onnx-to-onnx %s -split-input-file | FileCheck %s
  ...
  func.func @test_squeeze_pattern(%arg0: tensor<1x3xi8> {onnx.name = "input"} loc(unknown)) -> (tensor<1x1x3xi8> {onnx.name = "requantized"}) {
    ...
    %4 = "onnx.Squeeze"(%3, %2) {onnx_node_name = "onnx.Squeeze_1"} : (tensor<1x3xf32>, tensor<1xi64>) -> tensor<1x1x3xf32>
