...
module {
  ...
  // CHECK: onnx.Reshape{{.*}} -> tensor<1x32x1x128x!quant.uniform<i8:f32, 5.000000e-02>>
  // CHECK: onnx.Reshape{{.*}} -> tensor<128x1x1x1xf32>
  // CHECK: onnx.Conv{{.*}}group = 32{{.*}}kernel_shape = [1, 1]
  // CHECK: onnx.Relu
  ...
  // CHECK: onnx.Reshape{{.*}} -> tensor<8x256x1x1x!quant.uniform<i8:f32, 1.000000e-01>>
