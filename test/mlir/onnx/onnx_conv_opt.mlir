...
// CHECK-DAG:       [[VAR_1_:%.+]] = "onnx.LayoutTransform"([[IMAGE_]]) {layout = "UNDEFINED"} : (tensor<5x3x32x32xf32>) -> tensor<5x3x32x32xf32, #onnx.encoding<{dataLayout = "NCHW4C"}>>
// CHECK-DAG:       [[VAR_2_:%.+]] = "onnx.LayoutTransform"([[FILTER_]]) {layout = "UNDEFINED"} : (tensor<?x3x2x2xf32>) -> tensor<?x3x2x2xf32, #onnx.encoding<{dataLayout = "KCMN4C4K"}>>
// CHECK:           [[VAR_3_:%.+]] = "onnx.Conv"([[VAR_1_]], [[VAR_2_]], [[VAR_0_]]) {auto_pad = "NOTSET", group = 1 : si64, kernel_shape = [2, 2], pads = [0, 0, 0, 0]} : (tensor<5x3x32x32xf32, #onnx.encoding<{dataLayout = "NCHW4C"}>>, tensor<?x3x2x2xf32, #onnx.encoding<{dataLayout = "KCMN4C4K"}>>, none) -> tensor<5x?x31x31xf32>
// CHECK:           [[VAR_4_:%.+]] = "onnx.LayoutTransform"([[VAR_3_]]) {layout = "UNDEFINED"} : (tensor<5x?x31x31xf32>) -> tensor<5x?x31x31xf32>
