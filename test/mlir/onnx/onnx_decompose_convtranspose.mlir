...
// Test unit strides. Only convert weight tensor
  ...
  // CHECK-SAME:   ([[PARAM_0_:%.+]]: tensor<1x1x3x3xf32>, [[PARAM_1_:%.+]]: tensor<1x2x3x3xf32>)
    ...
    // CHECK-NOT: separator of consecutive DAGs
    // CHECK-DAG:       [[VAR_11_:%.+]] = "onnx.Pad"([[VAR_8_]], [[VAR_9_]], [[VAR_10_]]) {mode = "constant"} : (tensor<*xf32>, tensor<8xi64>, tensor<f32>) -> tensor<1x2x5x5xf32>
    // CHECK-DAG:       [[VAR_12_:%.+]] = onnx.Constant dense<0> : tensor<8xi64>
    // CHECK-DAG:       [[VAR_13_:%.+]] = onnx.Constant dense<0.000000e+00> : tensor<f32>
    // CHECK:           [[VAR_14_:%.+]] = "onnx.Pad"([[VAR_11_]], [[VAR_12_]], [[VAR_13_]]) {mode = "constant"} : (tensor<1x2x5x5xf32>, tensor<8xi64>, tensor<f32>) -> tensor<1x2x5x5xf32>
...
// Test 1d input
  ...
  // CHECK-SAME:   ([[PARAM_0_:%.+]]: tensor<1x1x3xf32>, [[PARAM_1_:%.+]]: tensor<1x2x3xf32>)
    ...
    // CHECK:           [[VAR_9_:%.+]] = "onnx.Pad"([[VAR_6_]], [[VAR_7_]], [[VAR_8_]]) {mode = "constant"} : (tensor<*xf32>, tensor<6xi64>, tensor<f32>) -> tensor<1x2x5xf32>
...
// Test 3d input
  ...
  // CHECK-SAME:   ([[PARAM_0_:%.+]]: tensor<1x1x3x4x5xf32>, [[PARAM_1_:%.+]]: tensor<1x2x3x3x3xf32>)
    ...
    // CHECK-DAG:       [[VAR_13_:%.+]] = onnx.Constant dense<0.000000e+00> : tensor<f32>
    // CHECK-NOT: separator of consecutive DAGs
    // CHECK-DAG:       [[VAR_14_:%.+]] = "onnx.Pad"([[VAR_11_]], [[VAR_12_]], [[VAR_13_]]) {mode = "constant"} : (tensor<*xf32>, tensor<10xi64>, tensor<f32>) -> tensor<1x2x5x6x7xf32>
    // CHECK-DAG:       [[VAR_15_:%.+]] = onnx.Constant dense<0> : tensor<10xi64>
    // CHECK-DAG:       [[VAR_16_:%.+]] = onnx.Constant dense<0.000000e+00> : tensor<f32>
    // CHECK-NOT: separator of consecutive DAGs
    // CHECK-DAG:       [[VAR_17_:%.+]] = "onnx.Pad"([[VAR_14_]], [[VAR_15_]], [[VAR_16_]]) {mode = "constant"} : (tensor<1x2x5x6x7xf32>, tensor<10xi64>, tensor<f32>) -> tensor<1x2x5x6x7xf32>
    // CHECK-DAG:       [[VAR_18_:%.+]] = onnx.Constant dense<0> : tensor<10xi64>
    // CHECK-DAG:       [[VAR_19_:%.+]] = onnx.Constant dense<0.000000e+00> : tensor<f32>
    // CHECK:           [[VAR_20_:%.+]] = "onnx.Pad"([[VAR_17_]], [[VAR_18_]], [[VAR_19_]]) {mode = "constant"} : (tensor<1x2x5x6x7xf32>, tensor<10xi64>, tensor<f32>) -> tensor<1x2x5x6x7xf32>
...
// Test non unit strides. Added pads between elements  in input data.
  ...
  // CHECK-SAME:   ([[PARAM_0_:%.+]]: tensor<1x1x3x3xf32>, [[PARAM_1_:%.+]]: tensor<1x2x3x3xf32>)
    ...
    // CHECK-DAG:       [[VAR_11_:%.+]] = onnx.Constant dense<0.000000e+00> : tensor<f32>
    // CHECK-NOT: separator of consecutive DAGs
    // CHECK-DAG:       [[VAR_12_:%.+]] = "onnx.Pad"([[VAR_9_]]#0, [[VAR_10_]], [[VAR_11_]]) {mode = "constant"} : (tensor<1x1x1x3xf32>, tensor<8xi64>, tensor<f32>) -> tensor<1x1x3x3xf32>
    ...
    // CHECK:           [[VAR_15_:%.+]] = "onnx.Pad"([[VAR_9_]]#1, [[VAR_13_]], [[VAR_14_]]) {mode = "constant"} : (tensor<1x1x1x3xf32>, tensor<8xi64>, tensor<f32>) -> tensor<1x1x3x3xf32>
    ...
    // CHECK-DAG:       [[VAR_20_:%.+]] = onnx.Constant dense<0.000000e+00> : tensor<f32>
    // CHECK-NOT: separator of consecutive DAGs
    // CHECK-DAG:       [[VAR_21_:%.+]] = "onnx.Pad"([[VAR_18_]]#0, [[VAR_19_]], [[VAR_20_]]) {mode = "constant"} : (tensor<1x1x7x1xf32>, tensor<8xi64>, tensor<f32>) -> tensor<1x1x7x2xf32>
    // CHECK-DAG:       [[VAR_22_:%.+]] = onnx.Constant dense<[0, 0, 0, 0, 0, 0, 0, 1]> : tensor<8xi64>
    // CHECK-DAG:       [[VAR_23_:%.+]] = onnx.Constant dense<0.000000e+00> : tensor<f32>
    // CHECK:           [[VAR_24_:%.+]] = "onnx.Pad"([[VAR_18_]]#1, [[VAR_22_]], [[VAR_23_]]) {mode = "constant"} : (tensor<1x1x7x1xf32>, tensor<8xi64>, tensor<f32>) -> tensor<1x1x7x2xf32>
    ...
    // CHECK-DAG:       [[VAR_28_:%.+]] = onnx.Constant dense<0.000000e+00> : tensor<f32>
    // CHECK-NOT: separator of consecutive DAGs
    // CHECK-DAG:       [[VAR_29_:%.+]] = "onnx.Pad"([[VAR_26_]], [[VAR_27_]], [[VAR_28_]]) {mode = "constant"} : (tensor<*xf32>, tensor<8xi64>, tensor<f32>) -> tensor<1x2x7x3xf32>
    // CHECK-DAG:       [[VAR_30_:%.+]] = onnx.Constant dense<0> : tensor<8xi64>
    // CHECK-DAG:       [[VAR_31_:%.+]] = onnx.Constant dense<0.000000e+00> : tensor<f32>
    // CHECK:           [[VAR_32_:%.+]] = "onnx.Pad"([[VAR_29_]], [[VAR_30_]], [[VAR_31_]]) {mode = "constant"} : (tensor<1x2x7x3xf32>, tensor<8xi64>, tensor<f32>) -> tensor<1x2x7x3xf32>
...
// Test output_padding. Additional pads are inserted after Conv op
  ...
  // CHECK-SAME:   ([[PARAM_0_:%.+]]: tensor<1x1x3x3xf32>, [[PARAM_1_:%.+]]: tensor<1x2x3x3xf32>)
    ...
    // CHECK-DAG:       [[VAR_28_:%.+]] = onnx.Constant dense<0.000000e+00> : tensor<f32>
    // CHECK-NOT: separator of consecutive DAGs
    // CHECK-DAG:       [[VAR_29_:%.+]] = "onnx.Pad"([[VAR_26_]], [[VAR_27_]], [[VAR_28_]]) {mode = "constant"} : (tensor<*xf32>, tensor<8xi64>, tensor<f32>) -> tensor<1x2x10x7xf32>
    // CHECK-DAG:       [[VAR_30_:%.+]] = onnx.Constant dense<[0, 0, 0, 0, 0, 0, 0, 1]> : tensor<8xi64>
    // CHECK-DAG:       [[VAR_31_:%.+]] = onnx.Constant dense<0.000000e+00> : tensor<f32>
    // CHECK:           [[VAR_32_:%.+]] = "onnx.Pad"([[VAR_29_]], [[VAR_30_]], [[VAR_31_]]) {mode = "constant"} : (tensor<1x2x10x7xf32>, tensor<8xi64>, tensor<f32>) -> tensor<1x2x10x8xf32>
...
// Test for unknown dimension in spatial dimensions
  ...
  // CHECK-SAME:   ([[PARAM_0_:%.+]]: tensor<?x?x3x3xf32>, [[PARAM_1_:%.+]]: tensor<?x?x3x3xf32>)
    ...
    // CHECK-DAG:       [[VAR_11_:%.+]] = onnx.Constant dense<0.000000e+00> : tensor<f32>
    // CHECK-NOT: separator of consecutive DAGs
    // CHECK-DAG:       [[VAR_12_:%.+]] = "onnx.Pad"([[VAR_9_]]#0, [[VAR_10_]], [[VAR_11_]]) {mode = "constant"} : (tensor<?x?x1x3xf32>, tensor<8xi64>, tensor<f32>) -> tensor<?x?x3x3xf32>
    // CHECK-DAG:       [[VAR_13_:%.+]] = onnx.Constant dense<[0, 0, 0, 0, 0, 0, 2, 0]> : tensor<8xi64>
    // CHECK-DAG:       [[VAR_14_:%.+]] = onnx.Constant dense<0.000000e+00> : tensor<f32>
    // CHECK:           [[VAR_15_:%.+]] = "onnx.Pad"([[VAR_9_]]#1, [[VAR_13_]], [[VAR_14_]]) {mode = "constant"} : (tensor<?x?x1x3xf32>, tensor<8xi64>, tensor<f32>) -> tensor<?x?x3x3xf32>
    ...
    // CHECK-DAG:       [[VAR_20_:%.+]] = onnx.Constant dense<0.000000e+00> : tensor<f32>
    // CHECK-NOT: separator of consecutive DAGs
    // CHECK-DAG:       [[VAR_21_:%.+]] = "onnx.Pad"([[VAR_18_]]#0, [[VAR_19_]], [[VAR_20_]]) {mode = "constant"} : (tensor<?x?x7x1xf32>, tensor<8xi64>, tensor<f32>) -> tensor<?x?x7x2xf32>
    // CHECK-DAG:       [[VAR_22_:%.+]] = onnx.Constant dense<[0, 0, 0, 0, 0, 0, 0, 1]> : tensor<8xi64>
    // CHECK-DAG:       [[VAR_23_:%.+]] = onnx.Constant dense<0.000000e+00> : tensor<f32>
    // CHECK:           [[VAR_24_:%.+]] = "onnx.Pad"([[VAR_18_]]#1, [[VAR_22_]], [[VAR_23_]]) {mode = "constant"} : (tensor<?x?x7x1xf32>, tensor<8xi64>, tensor<f32>) -> tensor<?x?x7x2xf32>
    ...
    // CHECK-DAG:       [[VAR_28_:%.+]] = onnx.Constant dense<0.000000e+00> : tensor<f32>
    // CHECK-NOT: separator of consecutive DAGs
    // CHECK-DAG:       [[VAR_29_:%.+]] = "onnx.Pad"([[VAR_26_]], [[VAR_27_]], [[VAR_28_]]) {mode = "constant"} : (tensor<*xf32>, tensor<8xi64>, tensor<f32>) -> tensor<?x?x10x7xf32>
    // CHECK-DAG:       [[VAR_30_:%.+]] = onnx.Constant dense<[0, 0, 0, 0, 0, 0, 0, 1]> : tensor<8xi64>
    // CHECK-DAG:       [[VAR_31_:%.+]] = onnx.Constant dense<0.000000e+00> : tensor<f32>
    // CHECK:           [[VAR_32_:%.+]] = "onnx.Pad"([[VAR_29_]], [[VAR_30_]], [[VAR_31_]]) {mode = "constant"} : (tensor<?x?x10x7xf32>, tensor<8xi64>, tensor<f32>) -> tensor<?x?x10x8xf32>
