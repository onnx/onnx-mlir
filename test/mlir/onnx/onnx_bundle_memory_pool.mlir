// RUN: onnx-mlir-opt --shape-inference --convert-onnx-to-krnl='opt-level=3' --buffer-deallocation --enable-memory-pool --bundle-memory-pools --canonicalize %s -split-input-file | FileCheck %s
