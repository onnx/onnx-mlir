...
static const llvm::StringRef SHARED_LIB_BASE("./TestStackedLSTM_main_graph");
...
// Returns whether onnx-mlir compiled Stacked-LSTM is producing the same
// results as a naive implementation of LSTM for a specific set of LSTM
// parameters/configuration.
bool isOMStackedLSTMTheSameAsNaiveImplFor(const int direction, const int S,
...
int main(int argc, char *argv[]) {
  ...
  // 1: forward, -1: reverse, 2: bidirectional
    ...
    RC_ASSERT(isOMStackedLSTMTheSameAsNaiveImplFor(D, S, B, I, H, isDynS == 0,
              ...
              for (int64_t noneh = 0; noneh < 2; noneh++)
                for (int64_t nonec = 0; nonec < 2; nonec++)
                  for (int64_t nonep = min_nonep; nonep < 2; nonep++) {
                    // forward
                    assert(isOMStackedLSTMTheSameAsNaiveImplFor(
