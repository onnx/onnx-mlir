...
int main(int argc, char *argv[]) {
  ...
  bool success = rc::check("GRU implementation correctness", []() {
    ...
    // LinearBeforeReset.
    // zDNN supports only when L is 1.
    const auto L = 1;
  ...
  // zDNN supports only when linearBeforeReset is 1.
    ...
    for (int64_t b = 3; b < 4; b++)
      for (int64_t i = 2; i < 5; i++)
        for (int64_t h = 2; h < 5; h++) {
          // Static dimensions.
          // forward
          assert(isOMGRUTheSameAsNaiveImplFor(1, s, b, i, h, 1));
          // reverse
          assert(isOMGRUTheSameAsNaiveImplFor(-1, s, b, i, h, 1));
          // bidirectional
          // assert(isOMGRUTheSameAsNaiveImplFor(2, s, b, i, h, 1));

          // Dynamic dimensions for sequence, batch size.
          // forward
          assert(isOMGRUTheSameAsNaiveImplFor(1, s, b, i, h, 1, true, true));
          // reverse
          assert(isOMGRUTheSameAsNaiveImplFor(-1, s, b, i, h, 1, true, true));
          // bidirectional
          // assert(isOMGRUTheSameAsNaiveImplFor(2, s, b, i, h, 1, true, true));
        }
  return 0;
}
