...
{
  using DMemArray2D = npl::ArrayRef<2, npl::MemoryKind::DeviceMem, npl::ElementDataType::BFloat16, npl::FormatKind::Tile>;
...
{
    const auto NativeMatrixDim = Trainwave::FirmwareRT::GetHWInfo().GetNativeMatrixDim();

    npl::NotificationList done = npl::BuildNotificationList(npl::NotifConsumerKind::ClusterCP);

    auto input1 = reinterpret_cast<KernelParams::ArrayRef const*>(args.params.data);
    ...
    const Apollo::Primitives::SemId controlSemId
        = twf::SemManager::MapClusterSemOffset(twf::SemManager::GetNumClusterSem() - 1);

    DebugPuts("DeviceCP: Calling TileCP...\n"); 
    twfCluster::template CallTileCPAsync<mmTileCP::main_graph>(cmdArgs, controlSemId);
...
} // namespace MsgcMnistDemo::ClusterCP
